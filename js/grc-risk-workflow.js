...
(function(){
  ...
  function canAccessRiskIncidentRegisters(){var r=role(),p=Array.isArray(window._fbPerms)?window._fbPerms:[];return ['super_admin','admin','department_manager','dept_manager','risk_owner','platform_owner','governance_performance_manager'].indexOf(r)>=0||p.indexOf('edit_risk_management')>=0||p.indexOf('edit_incident_register')>=0||p.indexOf('*')>=0;}
...
`;document.head.appendChild(st);
  ...
  window._grcRiskOpenProfileMenu=function(ev){
    ...
    menu.innerHTML='<div class="qumc-profile-head"><div class="qumc-profile-avatar">'+esc(String(name).charAt(0).toUpperCase())+'</div><div style="min-width:0"><div class="qumc-profile-name">'+esc(name)+'</div><div class="qumc-profile-email">'+esc(email()||'—')+'</div></div></div>'+
      ...
      '<div class="grc-profile-task-panel"><div class="grc-profile-task-title">Requests</div>'+
        ...
        (canAccessRiskIncidentRegisters()?'<button class="grc-profile-task" onclick="document.getElementById(\'_grcUserProfileMenu\').remove();window._grcRiskOpenProfile()"><span>◇</span><div><strong>'+esc(approvalTaskTitle)+'</strong><small>'+esc(approvalTaskSub)+'</small></div><i id="_grcProfileRiskCount">'+approvalTaskCount+'</i></button>':'')+'</div>'+
  ...
  window._grcRiskShowDetails=function(id){
    ...
    ov.innerHTML=`<div class="grc-risk-dialog"><header><div><h2>${esc(r.requestCode||r.id)}</h2><p>${esc(operationLabel(r.operation,r))} · ${esc(statusLabel(r.status))}</p></div><button onclick="document.getElementById('_grcRiskDetailsOv').remove()">×</button></header><main>${content}${r.deleteReason?'<div class="grc-risk-note"><b>Deletion reason</b>'+esc(r.deleteReason)+'</div>':''}${historyTimeline(r,false)}${actions(r)}</main></div>`;
  ...
  window._grcRiskBindHeader=function(){
    if(String(window.__qumcActivePortal||'').toLowerCase()!=='grc')return;
    var root=document.getElementById('grcApp'),not=root&&root.querySelector('#grcRiskNotifBtn'),usr=root&&root.querySelector('.grc-profile-trigger');
  ...
  document.addEventListener('click',function(e){var p=document.getElementById('_grcRiskNotifPanel'),b=document.getElementById('grcRiskNotifBtn');if(p&&(!b||!b.contains(e.target))&&!p.contains(e.target))p.remove();var m=document.getElementById('_grcUserProfileMenu'),root=document.getElementById('grcApp'),u=root&&root.querySelector('.grc-profile-trigger');if(m&&(!u||!u.contains(e.target))&&!m.contains(e.target))m.remove();},true);
