...
(function(){
  ...
  function managerCanReviewRisk(r){if(!managerDepartmentRequest(r)||!responsibleRiskSubmitter(r))return false;var same=ownRequest(r);var submitterRole=String(r&&r.submittedByRole||'').toLowerCase().trim();return !same||['grc_owner','risk_owner','platform_owner'].indexOf(submitterRole)>=0;}
  ...
  function reviewManagerRequest(r){if(!isManager()||!r||r._managerAssigned!==true||String(r&&r.platform||'grc').toLowerCase()!=='grc'||reviewStage(r)!=='pending_department_manager')return false;var uid=String(window._fbUid||window.currentUserUid||'').trim(),rid=String(r&&r.requesterUid||'').trim();if(uid&&rid&&uid===rid)return false;var em=String(r&&r.userEmail||'').toLowerCase().trim(),me=email(),rr=String(r&&r.requesterRole||'').toLowerCase().trim().replace(/[ _-]+/g,'_');return !(em===me&&(!rr||rr==='department_manager'||rr==='dept_manager'));}
