...
(function(){
  ...
  /* A GRC/Risk/Platform Owner may legitimately submit with the same account that is currently acting as Department Manager in the test/combined-role workflow. Do not hide those routed department requests just because the email matches. */
  function managerCanReviewRisk(r){if(!managerDepartmentRequest(r)||!responsibleRiskSubmitter(r))return false;var same=ownRequest(r);var submitterRole=String(r&&r.submittedByRole||'').toLowerCase().trim().replace(/[ _-]+/g,'_');/* If the same account submitted while it was a normal User/Owner and was later promoted to Department Manager, the routed request is still a valid manager task. Block only a request that was originally submitted by the manager role itself. */return !same||submitterRole!=='department_manager';}
  ...
  function reviewManagerRequest(r){if(!isManager()||!r||r._managerAssigned!==true||String(r&&r.platform||'grc').toLowerCase()!=='grc'||reviewStage(r)!=='pending_department_manager')return false;var em=String(r&&r.userEmail||'').toLowerCase().trim(),me=email(),rr=String(r&&r.requesterRole||'').toLowerCase().trim().replace(/[ _-]+/g,'_');/* Do not reject the row just because UID/email matches: role can change after submission. Only suppress a request that was actually submitted as Department Manager. */return !(em===me&&rr==='department_manager');}
...
#_grcApprovalNoticeOv .grc-apn-btn.primary{background:#0f7f86;color:#fff}#_grcApprovalNoticeOv .grc-apn-btn.secondary{background:#eaf1f4;color:#294f61}#_grcApprovalNoticeOv .grc-apn-btn.good{background:#16803a;color:#fff}#_grcApprovalNoticeOv .grc-apn-btn.warn{background:#d99112;color:#fff}#_grcApprovalNoticeOv .grc-apn-btn.bad{background:#c9363f;color:#fff}
...
@media(max-width:650px){#_grcApprovalNoticeOv{padding:10px}#_grcApprovalNoticeOv .grc-apn-dialog{max-height:94vh;border-radius:17px}#_grcApprovalNoticeOv .grc-apn-meta{grid-template-columns:1fr}#_grcApprovalNoticeOv .grc-apn-summary{align-items:flex-start;flex-direction:column}}
      ...
      .grc-manager-approval-section .grc-risk-request-card{border:1px solid #d8e5ea;border-radius:14px;background:#fff;box-shadow:0 5px 16px rgba(23,63,95,.06);margin-bottom:10px;padding:13px}.grc-manager-approval-section .grc-risk-card-head{padding-bottom:9px;border-bottom:1px solid #edf2f4}.grc-manager-approval-section .grc-risk-card-grid{display:grid;grid-template-columns:125px minmax(0,1fr);gap:6px 12px;margin-top:10px}.grc-manager-approval-section .grc-risk-card-grid span{font-size:9px;color:#78909c;font-weight:800}.grc-manager-approval-section .grc-risk-card-grid b{font-size:10px;color:#294f61;overflow-wrap:anywhere}.grc-manager-approval-section .grc-risk-change-preview,.grc-manager-approval-section .grc-risk-history{margin-top:9px}.grc-manager-approval-section .grc-risk-details-btn{margin-top:9px}
...
`;document.head.appendChild(st);
  ...
  /* Backward-compatible alias for cached deployments that still call the old
     function name. This prevents a stale handler from breaking the whole notice. */
  function viewSuperAdminNoticeCard(r){return reviewSuperAdminNoticeCard(r);}
  ...
  function refreshManagerApprovalQueues(force){
    ...
    return Promise.allSettled([risk,review]).then(function(rows){
      ...
      if(rows[0].status==='fulfilled'){
        var riskPayload=rows[0].value;
        managerRiskAllRows=Array.isArray(riskPayload&&riskPayload.allRecords)?riskPayload.allRecords:(Array.isArray(riskPayload)?riskPayload:[]);
        cache=(Array.isArray(riskPayload&&riskPayload.records)?riskPayload.records:(Array.isArray(riskPayload)?riskPayload:[])).filter(managerDepartmentRequest);
      }else errors.push('Risk / Incident: '+String(rows[0].reason&&rows[0].reason.message||rows[0].reason||'Permission denied'));
      if(rows[1].status==='fulfilled'){
        var reviewPayload=rows[1].value;
        managerReviewAllRows=Array.isArray(reviewPayload&&reviewPayload.allRecords)?reviewPayload.allRecords:(Array.isArray(reviewPayload)?reviewPayload:[]);
        reviewApprovalRows=(Array.isArray(reviewPayload&&reviewPayload.records)?reviewPayload.records:(Array.isArray(reviewPayload)?reviewPayload:[])).filter(reviewManagerRequest);
  ...
  function actions(r){var s=String(r.status||''),html='';
    if(isManager()&&managerCanReviewRisk(r)&&s==='pending_manager')html='<button class="grc-risk-action good" onclick="window._grcRiskDecision(\''+esc(r.id)+'\',\'manager_approve\',this)">Approve to Super Admin</button><button class="grc-risk-action warn" onclick="window._grcRiskDecision(\''+esc(r.id)+'\',\'manager_return\',this)">Return to '+esc(submitterLabel(r))+'</button><button class="grc-risk-action bad" onclick="window._grcRiskDecision(\''+esc(r.id)+'\',\'manager_reject\',this)">Reject</button>';
    if(isManager()&&managerCanReviewRisk(r)&&s==='returned_manager')html='<button class="grc-risk-action good" onclick="window._grcRiskDecision(\''+esc(r.id)+'\',\'manager_approve\',this)">Approve to Super Admin</button><button class="grc-risk-action warn" onclick="window._grcRiskDecision(\''+esc(r.id)+'\',\'manager_return\',this)">Return to '+esc(submitterLabel(r))+'</button><button class="grc-risk-action bad" onclick="window._grcRiskDecision(\''+esc(r.id)+'\',\'manager_reject\',this)">Reject</button>';
  ...
  function reviewProfileCard(r){
    var details=String(r.details||r.title||r.relatedNewText||r.category||r.relatedType||'').trim();
    var st=String(r.workflowStage||r.status||'').toLowerCase();
    var statusText={pending_department_manager:isAr()?'بانتظار موافقة مدير القسم':'Pending Department Manager Approval',pending_super_admin:isAr()?'بانتظار مراجعة السوبر أدمن':'Pending Super Admin Review',returned_requester:isAr()?'معاد لمقدم الطلب':'Returned to Requester',rejected_manager:isAr()?'مرفوض من مدير القسم':'Rejected by Department Manager',rejected_super_admin:isAr()?'مرفوض من السوبر أدمن':'Rejected by Super Admin',closed:isAr()?'مغلق':'Closed',published:isAr()?'منشور':'Published'}[st]||String(r.status||r.workflowStage||'—').replace(/_/g,' ');
    var statusClass=/rejected|closed|cancelled/.test(st)?'bad':/^pending/.test(st)?'warn':'good';
    var canAct=st==='pending_department_manager';
