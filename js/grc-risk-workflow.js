...
`;document.head.appendChild(st);
  ...
  function renderProfileBody(){
    ...
    if(isManager()){
      ...
      var riskRows=managerRiskAllRows.filter(function(r){return !ownRequest(r)&&responsibleRiskSubmitter(r);}).sort(function(a,b){return (new Date(b.updatedAtIso||b.createdAtIso||b.createdAt||0).getTime()||0)-(new Date(a.updatedAtIso||a.createdAtIso||a.createdAt||0).getTime()||0);});
  ...
  window._grcRiskOpenProfileMenu=function(ev){
    ...
    var approvalTaskTitle='Risk & Incident Registers',approvalTaskSub=isManager()?'Risk & Incident requests for your department':'Approval requests and publication status',approvalTaskCount=(isManager()?cache.filter(function(r){return managerDepartmentRequest(r)&&!ownRequest(r)&&responsibleRiskSubmitter(r)&&String(r.status||'').toLowerCase()==='pending_manager';}).length+reviewApprovalRows.filter(reviewManagerRequest).length:cache.filter(actionable).length);
