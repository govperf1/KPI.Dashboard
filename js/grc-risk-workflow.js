...
`;document.head.appendChild(st);
  ...
  window._grcRespondSystemRequest=function(id,status){var comment='';if(status==='rejected'){comment=window.prompt('Enter the rejection reason:','');if(comment===null)return;if(!String(comment).trim()){window.alert('A rejection reason is required.');return;}}window._grcRequestsRespond(id,status,comment).then(function(){var x=document.getElementById('grcMyReqOv');if(x)x.remove();window._grcShowMyRequests();}).catch(function(err){window.alert(String(err&&err.message||err));});};
