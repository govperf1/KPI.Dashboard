/* ======================================================================
   QUMC GRC1 — Excel / Report / Page Export
...
(function(){
  ...
  function snap(){return typeof window._grc1GetExportSnapshot==='function'?(window._grc1GetExportSnapshot()||{}):{};}
  function modules(){return typeof window._grc1GetModules==='function'?window._grc1GetModules():Object.keys(PAGE_LABELS).map(function(id){return{id:id,label:PAGE_LABELS[id]};});}
  ...
  function overlay(title,sub,body,confirmLabel,confirmFn){
    ...
    ov.innerHTML='<div class="grc-export-dialog"><div class="grc-export-dialog-head"><div><h2>'+esc(title)+'</h2><p>'+esc(sub||'')+'</p></div><button type="button" onclick="window._grc1CloseExportDialog()">×</button></div><div class="grc-export-dialog-body">'+body+'<div class="grc-export-actions-row"><button type="button" class="grc-export-cancel" onclick="window._grc1CloseExportDialog()">Cancel</button><button type="button" class="grc-export-confirm" onclick="'+confirmFn+'">'+esc(confirmLabel)+'</button></div></div></div>';
    document.body.appendChild(ov);ov.addEventListener('click',function(e){if(e.target===ov)closeOverlay();});
  }
  window._grc1CloseExportDialog=closeOverlay;
  ...
  function registerGroupHtml(group){return'<div class="grc-export-compact-group"><button type="button" class="grc-export-dropdown-btn" onclick="window._grc1ToggleExcelDropdown(\''+group.id+'\',event)"><span><b>'+esc(group.title)+'</b><small id="grcExcelDropLabel-'+group.id+'">All selected</small></span><i>⌄</i></button><div id="grcExcelDrop-'+group.id+'" class="grc-export-dropdown-menu"><label class="grc-export-check group-all"><input id="grcExcelGroupAll-'+group.id+'" type="checkbox" checked onchange="window._grc1ToggleRegisterGroup(\''+group.id+'\',this.checked)"><span>Select all in this group</span></label>'+group.items.map(function(item){return'<label class="grc-export-check"><input data-grc-excel-selectable data-grc-register-group="'+group.id+'" type="checkbox" name="grcExcelRegister" value="'+item[0]+'" checked onchange="window._grc1SyncRegisterGroup(\''+group.id+'\')"><span>'+esc(item[1])+'</span></label>';}).join('')+'</div></div>';}
  window._grc1ToggleExcelDropdown=function(id,e){if(e){e.preventDefault();e.stopPropagation();}document.querySelectorAll('.grc-export-dropdown-menu.is-open').forEach(function(x){if(x.id!=='grcExcelDrop-'+id)x.classList.remove('is-open');});var m=document.getElementById('grcExcelDrop-'+id);if(m)m.classList.toggle('is-open');};
  window._grc1ToggleExcelAll=function(on){excelSelectable().forEach(function(x){x.checked=!!on;});REGISTER_GROUPS.forEach(function(g){var all=document.getElementById('grcExcelGroupAll-'+g.id);if(all)all.checked=!!on;updateDropdownLabel(g);});};
  window._grc1ToggleRegisterGroup=function(group,on){document.querySelectorAll('input[data-grc-register-group="'+group+'"]').forEach(function(x){x.checked=!!on;});window._grc1SyncRegisterGroup(group);};
  window._grc1SyncRegisterGroup=function(group){var boxes=Array.prototype.slice.call(document.querySelectorAll('input[data-grc-register-group="'+group+'"]')),all=document.getElementById('grcExcelGroupAll-'+group),g=REGISTER_GROUPS.find(function(x){return x.id===group;});if(all){all.checked=boxes.length>0&&boxes.every(function(x){return x.checked;});all.indeterminate=boxes.some(function(x){return x.checked;})&&!all.checked;}if(g)updateDropdownLabel(g);window._grc1SyncExcelAll();};
  window._grc1SyncExcelAll=function(){var boxes=excelSelectable(),all=document.getElementById('grcExcelSelectAll');if(all){all.checked=boxes.length>0&&boxes.every(function(x){return x.checked;});all.indeterminate=boxes.some(function(x){return x.checked;})&&!all.checked;}};
  window._grc1OpenExcelSelector=function(){
    ...
    var deptSection='';
    if(canViewAll){
      var deptChecks=DEPTS.map(function(d){return'<label class="grc-export-check"><input data-grc-excel-selectable type="checkbox" name="grcExcelDept" value="'+d[0]+'" checked onchange="window._grc1SyncExcelAll()"><span>'+d[1]+'</span></label>';}).join('');
    ...
    var body='<label class="grc-export-select-all"><input id="grcExcelSelectAll" type="checkbox" checked onchange="window._grc1ToggleExcelAll(this.checked)"><span><b>Select All Options</b><small>'+help+'</small></span></label>'+deptSection+'<div class="grc-export-section"><h3>Registers</h3><p class="grc-export-help">Open a compact dropdown and choose one or more register tables.</p><div class="grc-export-dropdown-grid">'+groups+'</div></div>';
    overlay('GRC1 Excel Export',canViewAll?'Choose departments and register tables. Every selected table is exported from its register data.':'Choose register tables for your assigned department.',body,'Generate Excel','window._grc1GenerateExcel()');
  };
  ...
  function logoBase64(){var im=document.querySelector('#grc1App .grc-logo img')||document.getElementById('logoImg');if(!im||!/^data:image/.test(im.src||''))return null;return im.src;}
  ...
  async function buildExcelJs(sets,depts){
    var wb=new ExcelJS.Workbook();wb.creator='QUMC GRC1 Workspace';wb.created=new Date();var logo=logoBase64(),logoId=null;if(logo){try{logoId=wb.addImage({base64:logo,extension:logo.indexOf('png')>=0?'png':'jpeg'});}catch(_){}}
  ...
  window._grc1GenerateExcel=async function(){
    ...
    closeOverlay();
    try{var hasExcelJs=await waitForExcelJs(7000);if(hasExcelJs)await buildExcelJs(sets,depts);else buildSheetJs(sets,depts);if(typeof window._grc1RecordAuditDirect==='function')window._grc1RecordAuditDirect('GRC1_EXPORT_EXCEL','Exported GRC1 registers: '+selected.join(', '));}catch(e){alert('Excel export failed: '+String(e&&e.message||e));}
  };
  ...
  function parsePage(id){
    ...
    tmp.innerHTML=typeof window._grc1GetPageHtml==='function'?window._grc1GetPageHtml(id):'';
    return tmp;
  }
  ...
  async function ensureExecutiveRendered(){
    ...
    if(live)return live;
    if(typeof window._grc1Switch==='function')window._grc1Switch('executive');
  ...
  function reportCanViewAllDepartments(){var r=reportRole(),p=Array.isArray(window._fbPerms)?window._fbPerms:[];if(typeof window._grc1CanViewAllDepartments==='function')return !!window._grc1CanViewAllDepartments();return r==='admin'||r==='super_admin'||p.indexOf('*')>=0||p.indexOf('view_all_departments')>=0||p.indexOf('view_grc1_all_departments')>=0;}
  function reportUserDept(){var d='';try{d=typeof window._grc1GetCurrentDepartment==='function'?window._grc1GetCurrentDepartment():window._fbDept||window.currentUserDept||'';}catch(_){}d=String(d||'').toLowerCase();if(d.indexOf('safe')>=0)return'safety';if(d.indexOf('maint')>=0)return'maintenance';if(d.indexOf('laund')>=0)return'laundry';if(d.indexOf('house')>=0)return'housekeeping';if(d.indexOf('project')>=0)return'projects';return'';}
  ...
  function reportGroupHtml(group){return'<div class="grc-export-compact-group"><button type="button" class="grc-export-dropdown-btn" onclick="window._grc1ToggleReportDropdown(\''+group.id+'\',event)"><span><b>'+esc(group.title)+'</b><small id="grcReportDropLabel-'+group.id+'">All selected</small></span><i>⌄</i></button><div id="grcReportDrop-'+group.id+'" class="grc-export-dropdown-menu"><label class="grc-export-check group-all"><input id="grcReportGroupAll-'+group.id+'" type="checkbox" checked onchange="window._grc1ToggleReportGroup(\''+group.id+'\',this.checked)"><span>Select all in this section</span></label>'+group.items.map(function(item){return'<label class="grc-export-check"><input data-grc-report-selectable data-grc-report-group="'+group.id+'" type="checkbox" name="grcReportItem" value="'+item[0]+'" checked onchange="window._grc1SyncReportGroup(\''+group.id+'\')"><span>'+esc(item[1])+'</span></label>';}).join('')+'</div></div>';}
  window._grc1ToggleReportDropdown=function(id,e){if(e){e.preventDefault();e.stopPropagation();}document.querySelectorAll('.grc-export-dropdown-menu.is-open').forEach(function(x){if(x.id!=='grcReportDrop-'+id)x.classList.remove('is-open');});var m=document.getElementById('grcReportDrop-'+id);if(m)m.classList.toggle('is-open');};
  window._grc1ToggleReportAll=function(on){reportSelectable().forEach(function(x){x.checked=!!on;});REPORT_GROUPS.forEach(function(g){var all=document.getElementById('grcReportGroupAll-'+g.id);if(all)all.checked=!!on;updateReportDropdownLabel(g);});};
  window._grc1ToggleReportGroup=function(group,on){document.querySelectorAll('input[data-grc-report-group="'+group+'"]').forEach(function(x){x.checked=!!on;});window._grc1SyncReportGroup(group);};
  window._grc1SyncReportGroup=function(group){var boxes=Array.prototype.slice.call(document.querySelectorAll('input[data-grc-report-group="'+group+'"]')),all=document.getElementById('grcReportGroupAll-'+group),g=REPORT_GROUPS.find(function(x){return x.id===group;});if(all){all.checked=boxes.length>0&&boxes.every(function(x){return x.checked;});all.indeterminate=boxes.some(function(x){return x.checked;})&&!all.checked;}if(g)updateReportDropdownLabel(g);window._grc1SyncReportAll();};
  window._grc1SyncReportAll=function(){var boxes=reportSelectable(),all=document.getElementById('grcReportSelectAll');if(all){all.checked=boxes.length>0&&boxes.every(function(x){return x.checked;});all.indeterminate=boxes.some(function(x){return x.checked;})&&!all.checked;}};
  window._grc1OpenReportSelector=function(){
    ...
    if(canAll){var deptChecks=DEPTS.map(function(d){return'<label class="grc-export-check"><input data-grc-report-selectable type="checkbox" name="grcReportDept" value="'+d[0]+'" checked onchange="window._grc1SyncReportAll()"><span>'+d[1]+'</span></label>';}).join('');deptSection='<div class="grc-export-section"><h3>Departments</h3><div class="grc-export-check-grid">'+deptChecks+'</div></div>';}
    var body='<label class="grc-export-select-all"><input id="grcReportSelectAll" type="checkbox" checked onchange="window._grc1ToggleReportAll(this.checked)"><span><b>Select All Options</b><small>Select or clear every available department and report section.</small></span></label>'+deptSection+'<div class="grc-export-section"><h3>Report Content</h3><p class="grc-export-help">Choose the executive-summary sections and indicators to include. The report uses cards and charts only; register tables are excluded.</p><div class="grc-export-dropdown-grid">'+REPORT_GROUPS.map(reportGroupHtml).join('')+'</div></div>';
    overlay('GRC1 Executive Summary','Choose the departments and the executive-summary content to include.',body,'Build Report','window._grc1GenerateReport()');
  };
  function logoSrc(){return((document.querySelector('#grc1App .grc-logo img')||document.getElementById('logoImg')||{}).src||'');}
...
`;
  ...
  async function buildAllPrintHost(ids){var app=document.getElementById('grc1App'),host=document.getElementById('grcNativeExportHost');if(host)host.remove();host=document.createElement('div');host.id='grcNativeExportHost';host.className='grc-native-export-host';for(var i=0;i<ids.length;i++){if(typeof window._grc1Switch==='function')window._grc1Switch(ids[i]);await wait(520);var live=document.getElementById('grc-page-'+ids[i]);if(!live)continue;var clone=cloneRenderedNode(live);freezeRenderedLayout(live,clone);cleanPrintNode(clone);clone.classList.add('is-active','grc-native-export-sheet');clone.setAttribute('data-grc-export-sheet',ids[i]);clone.style.display='block';host.appendChild(clone);}app.appendChild(host);return host;}
  function cleanupNativeExport(returnPage){document.body.classList.remove('grc-native-export','grc-native-export-all');document.body.removeAttribute('data-grc-export-page');var h=document.getElementById('grcNativeExportHost');if(h)h.remove();if(returnPage&&typeof window._grc1Switch==='function')window._grc1Switch(returnPage);}
  window._grc1ExportPage=async function(id){var menu=document.getElementById('grcExportMenu');if(menu)menu.classList.remove('is-open');var current=(document.querySelector('#grc1App .grc-page.is-active')||{}).id.replace(/^grc-page-/,'')||'executive';try{var ids=id==='all'?modules().map(function(x){return x.id;}):[id];await buildAllPrintHost(ids);document.body.classList.add('grc-native-export-all');document.body.setAttribute('data-grc-export-page',id);var done=function(){window.removeEventListener('afterprint',done);cleanupNativeExport(current);};window.addEventListener('afterprint',done);setTimeout(function(){window.print();},120);setTimeout(function(){if(document.body.classList.contains('grc-native-export')||document.body.classList.contains('grc-native-export-all'))cleanupNativeExport(current);},15000);if(typeof window._grc1RecordAuditDirect==='function')window._grc1RecordAuditDirect('GRC1_PAGE_EXPORT','Exported GRC1 page: '+id);}catch(e){cleanupNativeExport(current);alert('Export failed: '+String(e&&e.message||e));}};
})();
