...
window._selectPortal=async portal=>{
    ...
    window._qumcAssertFirestoreRulesV60=_grcRiskAssertRulesVersion;window._qumcAssertFirestoreRulesV43=_grcRiskAssertRulesVersion;window._qumcAssertFirestoreRulesV42=_grcRiskAssertRulesVersion;window._qumcAssertFirestoreRulesV41=_grcRiskAssertRulesVersion;
    ...
    window._grcRiskRequestResubmit=async function(requestId,proposedRecord,note){
      if(!_grcRiskCanSubmit('risk')&&!_grcRiskCanSubmit('incident'))throw new Error('Access denied.');
      await _grcRiskAssertRulesVersion();
      const ref=doc(db,GRC_RISK_REQUESTS_COLLECTION,requestId),snap=await getDoc(ref);
      if(!snap.exists())throw new Error('Request not found.');
      const r=snap.data();
      if(!_grcRiskOwnsRequest(r))throw new Error('Access denied.');
      ...
      /* The Department Manager decides exactly which fields may be changed.
         ...
         non-returned value from the stored proposal and merge only the selected
         fields. This keeps the workflow secure even if a caller bypasses the UI. */
      ...
      const updates={
        ...
        /* Preserve the manager's requested fields/note as immutable workflow context. */
