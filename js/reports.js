...
function renderReportArabic(){
  ...
  if(_rptRole==='kpi_owner'&&Array.isArray(_rptAssigned)&&_rptAssigned.length>0) ks_base=ks_base.filter(k=>_rptAssigned.includes(k.id));
  ...
  const deptOpts=(!_rptLocked?'<option value=""'+(window._rptDept===''?' selected':'')+'>كل الأقسام</option>':'')+
    Object.keys(DM).filter(d=>!_rptLocked||d===_rptLocked).map(d=>'<option value="'+d+'"'+(window._rptDept===d?' selected':'')+'>'+(DM[d].ar||DM[d].en)+'</option>').join('');
  const ks_dept=window._rptDept?ks_all.filter(k=>k.dept===window._rptDept):ks_all;
  const kpiOpts='<option value=""'+(window._rptKpi===''?' selected':'')+'>— كل المؤشرات —</option>'+ks_dept.map(k=>'<option value="'+k.id+'"'+(window._rptKpi===k.id?' selected':'')+'>'+k.id+' — '+(k.nameAr||k.nameEn)+'</option>').join('');
  ...
  const deptFull=rptDm?((rptDm.ar||rptDm.en)+''):'كل الأقسام';
  const metKs=ks.filter(k=>ok(k)===true), missKs=ks.filter(k=>ok(k)===false);
  ...
  const kpiYear=k1?String(k1.yr):(F.year!=='all'?String(F.year):'كل السنوات');
  const today=new Date();
  const dateStr=today.toLocaleDateString('ar-SA',{day:'2-digit',month:'long',year:'numeric'});
  const period=kpiYear+(F.qtr.includes('all')?'':' ('+F.qtr.map(q=>q.toUpperCase()).join('، ')+')');
  const kpiNameFull=ks.map(k=>k.nameAr||k.nameEn).join('، ')||'تقرير أداء المؤشرات';
  const kpiNameShort=k1?(k1.nameAr||k1.nameEn):(ks.length===0?'تقرير أداء المؤشرات':ks.length<=2?kpiNameFull:ks.length+' مؤشرات');
  ...
  let _eid=0;
  const EP=(txt,style='')=>{const id='ep'+(++_eid); const key='rpt_'+(window._rptKpi||'_all_')+'_'+_eid; const saved=(ST.rptEdits||{})[key]; let content=txt; if(saved&&typeof saved==='object'&&saved.ar)content=saved.ar; return '<div class="rpt-ep" id="'+id+'-w" style="display:flex;align-items:flex-start;gap:8px;'+style+'"><p id="'+id+'" dir="rtl" style="font-size:12px;color:#334155;line-height:1.9;margin:0;min-height:20px;flex:1;text-align:right">'+content+'</p><div id="'+id+'-acts" style="flex-shrink:0;display:flex;flex-direction:column;gap:4px;padding-top:2px"><button class="rpt-edit-btn" onclick="rptStartEdit(\''+id+'\')" title="تعديل" style="position:static;opacity:0;transform:translateX(4px)">✎</button></div></div>';};
  const QR=(q,v)=>{const ok2=v!==null&&v>=kpiTarget;return '<tr><td style="padding:9px 14px;font-weight:700;color:#152538;border:1px solid #E2E8F0">'+q+'</td><td style="padding:9px 14px;text-align:center;font-weight:700;color:#1E40AF;border:1px solid #E2E8F0">'+kpiTarget+'%</td><td style="padding:9px 14px;text-align:center;font-weight:800;background:'+(v===null?'#F8FAFC':ok2?'#DCFCE7':'#FEE2E2')+';color:'+(v===null?'#CBD5E1':ok2?'#166534':'#991B1B')+';border:1px solid #E2E8F0">'+(v!==null?v+'%':'غير متاح')+'</td><td style="padding:9px 14px;text-align:center;border:1px solid #E2E8F0"><span style="padding:3px 12px;border-radius:20px;font-size:10px;font-weight:700;background:'+(v===null?'#F8FAFC':ok2?'#ECFDF5':'#FEF2F2')+';color:'+(v===null?'#94A3B8':ok2?'#06845A':'#C42B2B')+'">'+(v===null?'قيد المتابعة':ok2?'محقق':'أقل من الهدف')+'</span></td></tr>';};
  const qBullets=[[q1v,'Q1'],[q2v,'Q2'],[q3v,'Q3'],[q4v,'Q4']].filter(([v])=>v!==null).map(([v,q])=>'• <strong>'+q+':</strong> كانت النتيجة <strong style="color:'+(v>=kpiTarget?'#06845A':'#C42B2B')+'">'+v+'%</strong>'+(v<kpiTarget?'، أقل من الهدف '+kpiTarget+'%.':' — تم تحقيق الهدف.')).join('<br>');
  const recs=[missKs.length?'تنفيذ إجراءات تصحيحية موجهة لعدد <strong>'+missKs.length+'</strong> مؤشر أقل من الهدف.':null,'تعزيز المتابعة الدورية للنتائج وربطها بخطط التحسين.','تحديث تحليل الأسباب الجذرية للمؤشرات غير المحققة.','رفع جاهزية الفرق من خلال التدريب والمتابعة المستمرة.',metKs.length?'المحافظة على الممارسات الفعالة في المؤشرات المحققة وعددها <strong>'+metKs.length+'</strong>.':null].filter(Boolean);
  el.innerHTML='<div style="padding:0;background:var(--bg);min-height:100%;direction:rtl">'+
  '<div class="rpt-topbar" style="display:flex;align-items:center;gap:10px;padding:14px 20px;background:var(--bg);flex-wrap:wrap"><div style="display:flex;align-items:center;gap:8px"><label style="font-size:10px;font-weight:700;color:var(--t2);white-space:nowrap">القسم:</label><select class="msel" style="min-width:160px;height:34px;font-size:11px" onchange="window._rptDept=this.value;window._rptKpi=\'\';renderReport()">'+deptOpts+'</select></div><div style="width:1px;height:22px;background:var(--border)"></div><div style="display:flex;align-items:center;gap:8px"><label style="font-size:10px;font-weight:700;color:var(--t2);white-space:nowrap">المؤشر:</label><select class="msel" style="min-width:220px;height:34px;font-size:11px" onchange="window._rptKpi=this.value;renderReport()">'+kpiOpts+'</select></div><button onclick="window.print()" style="padding:7px 18px;background:#C42B2B;color:#fff;border:none;border-radius:8px;cursor:pointer;font-family:inherit;font-size:11px;font-weight:700">تصدير PDF</button><span style="margin-right:auto;font-size:10px;color:var(--t3)">'+period+' · '+dateStr+'</span></div>'+ 
  '<div id="rptDocument" style="background:#fff;max-width:860px;margin:0 auto 32px;border:1px solid #E2E8F0;box-shadow:0 8px 30px rgba(15,23,42,.08);direction:rtl;text-align:right"><div style="padding:38px 44px">'+
  '<div style="border-bottom:4px solid #0195af;padding-bottom:18px;margin-bottom:24px"><h1 style="font-size:24px;font-weight:900;color:#152538;margin:0">تقرير أداء المؤشرات</h1><p style="margin:8px 0 0;color:#64748B;font-size:13px">'+deptFull+' · '+kpiNameShort+' · '+period+'</p></div>'+ 
  '<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:24px"><div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;padding:14px"><b>إجمالي المؤشرات</b><div style="font-size:26px;font-weight:900;color:#152538">'+ks.length+'</div></div><div style="background:#ECFDF5;border:1px solid #A7F3D0;border-radius:10px;padding:14px"><b>محققة</b><div style="font-size:26px;font-weight:900;color:#06845A">'+metKs.length+'</div></div><div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:10px;padding:14px"><b>غير محققة</b><div style="font-size:26px;font-weight:900;color:#C42B2B">'+missKs.length+'</div></div><div style="background:#EFF6FF;border:1px solid #BFDBFE;border-radius:10px;padding:14px"><b>متوسط الأداء</b><div style="font-size:26px;font-weight:900;color:'+(isMet?'#06845A':'#C42B2B')+'">'+(kpiResult!==null?kpiResult+'%':'—')+'</div></div></div>'+ 
  SH(1,'الملخص التنفيذي')+EP('يعرض هذا التقرير أداء '+deptFull+' خلال الفترة '+period+' فيما يتعلق بمؤشر/مؤشرات '+kpiNameFull+'. تمت مقارنة النتائج الفعلية بالهدف السنوي وتحليل حالة الإنجاز والفجوات لدعم اتخاذ القرار والتحسين المستمر.')+
  SH(2,'بيانات المؤشر')+'<table style="width:100%;border-collapse:collapse;margin-bottom:20px"><tr><td style="padding:10px;background:#F8FAFC;border:1px solid #E2E8F0;font-weight:700">اسم المؤشر</td><td style="padding:10px;border:1px solid #E2E8F0">'+kpiNameFull+'</td></tr><tr><td style="padding:10px;background:#F8FAFC;border:1px solid #E2E8F0;font-weight:700">القسم</td><td style="padding:10px;border:1px solid #E2E8F0">'+deptFull+'</td></tr><tr><td style="padding:10px;background:#F8FAFC;border:1px solid #E2E8F0;font-weight:700">الهدف السنوي</td><td style="padding:10px;border:1px solid #E2E8F0">'+kpiTarget+'%</td></tr><tr><td style="padding:10px;background:#F8FAFC;border:1px solid #E2E8F0;font-weight:700">النتيجة</td><td style="padding:10px;border:1px solid #E2E8F0">'+(kpiResult!==null?kpiResult+'%':'غير متاح')+' — '+(kpiResult===null?'قيد المتابعة':isMet?'محقق':'أقل من الهدف')+'</td></tr></table>'+ 
  SH(3,'النتائج الربعية')+'<table style="width:100%;border-collapse:collapse;margin-bottom:20px"><thead><tr style="background:#152538;color:#fff"><th style="padding:9px;border:1px solid #E2E8F0">الربع</th><th style="padding:9px;border:1px solid #E2E8F0">الهدف</th><th style="padding:9px;border:1px solid #E2E8F0">النتيجة</th><th style="padding:9px;border:1px solid #E2E8F0">الحالة</th></tr></thead><tbody>'+QR('Q1',q1v)+QR('Q2',q2v)+QR('Q3',q3v)+QR('Q4',q4v)+'</tbody></table>'+ 
  SH(4,'تحليل الأداء')+EP(qBullets||'لا توجد نتائج ربعية كافية للتحليل في الفترة المحددة.')+
  '<div style="height:260px;margin:18px 0;border:1px solid #E2E8F0;border-radius:10px;padding:10px"><canvas id="rptBarChart"></canvas></div>'+ 
  SH(5,'التوصيات وفرص التحسين')+'<div style="display:flex;flex-direction:column;gap:8px">'+recs.map((r,i)=>'<div style="display:flex;gap:12px;padding:11px 15px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:9px"><div style="width:26px;height:26px;background:#152538;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:800;color:#fff;flex-shrink:0">'+(i+1)+'</div>'+EP(r,'font-size:11.5px;color:#334155;line-height:1.85;margin:0;flex:1')+'</div>').join('')+'</div>'+ 
  SH(6,'الخلاصة')+EP('بناءً على النتائج الحالية، فإن الأداء '+(isMet?'يحقق الهدف المطلوب ويعكس مستوى جيداً من الالتزام التشغيلي.':'يحتاج إلى متابعة وإجراءات تصحيحية لضمان الوصول إلى الهدف السنوي.')+' توصي إدارة الحوكمة والأداء باستمرار المتابعة الدورية وتحليل الفجوات وتحديث خطط التحسين حسب نتائج كل ربع.')+
  '<div style="margin-top:28px;background:#152538;border-radius:10px;padding:14px 22px;color:#fff"><b>مدينة جامعة القصيم الطبية</b><br><span style="font-size:10px;color:rgba(255,255,255,.65)">قسم الحوكمة والأداء · إدارة المرافق والسلامة · تاريخ الإنشاء: '+dateStr+'</span></div>'+ 
  '</div></div></div>';
  el.querySelectorAll('.rpt-ep').forEach(w=>{w.addEventListener('mouseenter',()=>{const b=w.querySelector('.rpt-edit-btn');if(b)b.style.opacity='1';});w.addEventListener('mouseleave',()=>{const b=w.querySelector('.rpt-edit-btn');if(b&&!w.dataset.editing)b.style.opacity='0';});});
  setTimeout(()=>{requestAnimationFrame(()=>{['rptBarChart','rptLineChart','rptDonut'].forEach(id=>{if(CH[id]){CH[id].destroy();delete CH[id];}});const qVals=[q1v,q2v,q3v,q4v];let lq=0;qVals.forEach((v,i)=>{if(v!==null)lq=i+1;});const aL=['Q1','Q2','Q3','Q4'].slice(0,lq||4);const aV=qVals.slice(0,lq||4);CH['rptBarChart']=mkChart('rptBarChart',{type:'bar',data:{labels:aL,datasets:[{label:'النتيجة',data:aV,backgroundColor:aV.map(v=>v===null?'#CBD5E1':v>=kpiTarget?'rgba(22,163,74,.72)':'rgba(220,38,38,.68)'),borderRadius:5,borderSkipped:false},{label:'الهدف',data:aL.map(()=>kpiTarget),type:'line',borderColor:'#C42B2B',borderWidth:2,borderDash:[6,4],pointRadius:0,fill:false,order:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'top',labels:{font:{size:10},boxWidth:10,padding:10}},tooltip:{callbacks:{label:c=>c.dataset.label+': '+c.parsed.y+'%'}}},scales:{y:{min:0,max:110,ticks:{callback:v=>v+'%',font:{size:10}},grid:{color:'rgba(20,35,80,.06)'}},x:{ticks:{font:{size:11,weight:'600'}},grid:{display:false}}}}});});},120);
}

function renderReport(){
  if(typeof lang!=='undefined' && lang==='ar') return renderReportArabic();
  const el=document.getElementById('page-report');if(!el)return;
  if(typeof window._rptKpi==='undefined')window._rptKpi='';
...
/* ==========================================================
   QUMC FINAL REPORT PRINT FIX V2
   Print only the formal report document, remove dashboard chrome,
   add clean page borders, and force report colors/backgrounds.
   Dashboard export print remains intact.
   ========================================================== */
(function(){
  if(window.__qumcReportPrintFixV2Applied) return;
  window.__qumcReportPrintFixV2Applied=true;

  function addPrintCSS(){
    var old=document.getElementById('qumc-report-print-only-css');
    if(old) old.remove();
    var css=document.createElement('style');
    css.id='qumc-report-print-only-css';
    css.textContent = [
      '@media print{',
      '@page{size:A4;margin:8mm 0 7mm 0!important}',
      ...
      '#qumcPrintReportPage{display:block!important;position:static!important;width:210mm!important;min-height:297mm!important;box-sizing:border-box!important;background:#fff!important;padding:10mm 9mm!important;margin:0!important}',
      '#qumcPrintReportPage .qumc-print-report-sheet{display:block!important;width:100%!important;max-width:none!important;margin:0!important;background:#fff!important;border:1.4px solid #94A3B8!important;border-radius:8px!important;box-shadow:none!important;overflow:hidden!important;box-sizing:border-box!important}',
      '#qumcPrintReportPage #rptDocument,#qumcPrintReportPage #reportDoc{display:block!important;width:100%!important;max-width:none!important;margin:0!important;background:#fff!important;border:0!important;border-radius:8px!important;box-shadow:none!important;overflow:hidden!important}',
      ...
      '#qumcPrintReportPage table{page-break-inside:auto!important}',
      '#qumcPrintReportPage tr,#qumcPrintReportPage .chartbox,#qumcPrintReportPage .rpt-section{break-inside:avoid!important;page-break-inside:avoid!important}',
      ...
      '#qumcPrintReportPage .qumc-print-text-keep,#qumcPrintReportPage .qumc-print-analysis-keep{height:auto!important;min-height:0!important;max-height:none!important;overflow:visible!important;margin-bottom:2mm!important}',
  ...
  function reportIsActive(){
    var exportOverride=document.getElementById('kpi-print-override');
    var reportPage=document.getElementById('page-report');
    return !!(reportPage && !exportOverride && (reportPage.classList.contains('on') || reportPage.offsetParent!==null));
  }

  function getReportDocument(){
    return document.getElementById('rptDocument') || document.getElementById('reportDoc');
  }

  function cleanupPrintClone(){
    try{document.body.classList.remove('qumc-print-report-only');}catch(e){}
    try{document.documentElement.classList.remove('qumc-print-report-only');}catch(e){}
  ...
  var nativePrint=window.__qumcNativePrint || window.print.bind(window);
  ...
  window.print=function(){
    if(reportIsActive()){
      if(prepareReportPrint()){
        setTimeout(function(){
          try{ nativePrint(); }
          finally{ setTimeout(cleanupPrintClone, 1200); }
        },120);
        return;
      }
    }
  ...
  window.addEventListener('afterprint', function(){
    if(document.body.classList.contains('qumc-print-report-only')) cleanupPrintClone();
  });
})();
...
(function(){
  ...
  function isArabic(){
    try{return (typeof lang!=='undefined' && lang==='ar') || document.documentElement.dir==='rtl';}catch(e){return false;}
  }
  function printUser(){
    var name='', email='', role='';
    try{name=String(window._fbName||window.currentUserName||'').trim();}catch(e){}
    try{email=String(window._fbUser||window._fbEmail||window.currentUserEmail||sessionStorage.getItem('qumc_user_email')||localStorage.getItem('qumc_user_email')||'').trim();}catch(e){}
    try{role=String(window._fbRole||window.currentUserRole||'').trim();}catch(e){}
  ...
  function reportPeriodText(){
    var txt='';
    try{
      var top=document.querySelector('#page-report .rpt-topbar span');
      if(top) txt=(top.textContent||'').trim();
    }catch(e){}
    if(txt) return txt;
    try{
      var year=(F&&F.year&&F.year!=='all')?F.year:'All years';
      var q=(F&&Array.isArray(F.qtr)&&!F.qtr.includes('all'))?F.qtr.map(function(x){return String(x).toUpperCase();}).join(', '):'All quarters';
      return year+' · '+q;
    }catch(e){return '';}
  }
  function makeMetaBar(){
    var ar=isArabic();
    var u=printUser();
    var now=new Date();
    var d;
    try{d=now.toLocaleString(ar?'ar-SA':'en-GB',{year:'numeric',month:'short',day:'2-digit',hour:'2-digit',minute:'2-digit'});}catch(e){d=now.toLocaleString();}
    var period=reportPeriodText();
    var labelPrintedBy=ar?'طبع بواسطة':'Printed by';
    var labelPrintedAt=ar?'وقت الطباعة':'Printed at';
    var labelPeriod=ar?'الفترة':'Period';
    var title=ar?'تقرير أداء المؤشرات الرسمي':'Official KPI Performance Report';
    var bits=[];
    bits.push('<div class="qumc-print-meta-title">'+esc(title)+'</div>');
    bits.push('<div><b>'+esc(labelPrintedBy)+':</b> '+esc(u.name)+(u.email?' <span>'+esc(u.email)+'</span>':'')+(u.role?' <span>'+esc(u.role)+'</span>':'')+'</div>');
    bits.push('<div><b>'+esc(labelPrintedAt)+':</b> '+esc(d)+'</div>');
    if(period) bits.push('<div><b>'+esc(labelPeriod)+':</b> '+esc(period)+'</div>');
    return '<div class="qumc-print-meta" dir="'+(ar?'rtl':'ltr')+'">'+bits.join('')+'</div>';
  }
  ...
  function addPrintCSS(){
    ...
    css.textContent = [
      ...
      '#qumcPrintReportPage:before{content:"";position:fixed!important;top:.7mm!important;right:.7mm!important;bottom:.7mm!important;left:.7mm!important;border:2.2px solid #152538!important;border-radius:1.6mm!important;pointer-events:none!important;z-index:999999!important}',
      ...
      '#qumcPrintReportPage tfoot{display:table-footer-group!important}',
      '#qumcPrintReportPage tr{break-inside:avoid!important;page-break-inside:avoid!important}',
      '#qumcPrintReportPage h1{color:#fff!important;font-size:20px!important;line-height:1.25!important;font-weight:900!important;visibility:visible!important;opacity:1!important}',
  ...
  function convertCanvasesToImages(original, clone){
    ...
    for(var i=0;i<cloneCanvases.length;i++){
      ...
      if(!oc || !cc) continue;
      try{
        var data=oc.toDataURL('image/png');
        if(!data || data==='data:,') continue;
        var img=document.createElement('img');
        img.className='qumc-print-chart-img';
        img.src=data;
        img.alt=cc.getAttribute('aria-label') || cc.id || 'Report chart';
        var rect=oc.getBoundingClientRect();
        if(rect && rect.width) img.style.maxWidth=Math.min(Math.ceil(rect.width),780)+'px';
        cc.parentNode && cc.parentNode.replaceChild(img,cc);
      }catch(e){
        try{cc.style.display='block';}catch(_e){}
      }
    }
  }

  function markAvoidBlocks(root){
    if(!root) return;
    try{
  ...
  function prepareExternal(sourceDoc,options){
    ...
    addPrintCSS();
    options=options||{};
    if(String(options.orientation||'').toLowerCase()==='landscape'){
      var orientationCss=document.createElement('style');orientationCss.id='qumc-report-orientation-css';
      orientationCss.textContent='@media print{@page{size:A4 landscape!important;margin:0!important}#qumcPrintReportPage{padding:10mm 9mm 9mm!important}#qumcPrintReportPage:before{top:.6mm!important;right:.6mm!important;bottom:.6mm!important;left:.6mm!important}}';
      document.head.appendChild(orientationCss);document.body.classList.add('qumc-print-report-landscape');document.documentElement.classList.add('qumc-print-report-landscape');
    }
  ...
  window.print=function(){
    if(reportIsActive()){
      if(prepareReportPrint()){
        setTimeout(function(){
          try{ nativePrint(); }
          finally{ setTimeout(cleanupPrintClone, 1500); }
        },180);
        return;
      }
    }
...
/* ==========================================================
   QUMC REPORT PRINT FIX V5
   - Supersedes older report print wrappers
   - Keeps report content continuous without blank cover pages
   - Makes charts visible by converting canvases to stable images
   - Keeps printed by / printed at at the very end of the report
   ========================================================== */
(function(){
  if(window.__qumcReportPrintFixV5Applied) return;
  window.__qumcReportPrintFixV5Applied=true;

  function esc(s){
    return String(s==null?'':s).replace(/[&<>\'\"]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','\"':'&quot;'}[c];});
  }
  function isArabic(){
    try{return (typeof lang!=='undefined' && lang==='ar') || document.documentElement.dir==='rtl' || document.body.dir==='rtl';}catch(e){return false;}
  }
  function getReportDocument(){
    return document.getElementById('rptDocument') || document.getElementById('reportDoc');
  }
  function reportIsActive(){
    ...
    var reportPage=document.getElementById('page-report');
    var doc=getReportDocument();
    return !!(doc && reportPage && !exportOverride && (reportPage.classList.contains('on') || reportPage.offsetParent!==null));
  }
  function getUserInfoForPrint(){
    var name='', email='', role='';
    try{name=String(window._fbName||window.currentUserName||'').trim();}catch(e){}
    try{email=String(window._fbUser||window._fbEmail||window.currentUserEmail||sessionStorage.getItem('qumc_user_email')||localStorage.getItem('qumc_user_email')||'').trim();}catch(e){}
    try{role=String(window._fbRole||window.currentUserRole||'').trim();}catch(e){}
    if(!name && email) name=email.split('@')[0];
    if(!name) name='Governance and Performance Department';
    return {name:name,email:email,role:role};
  }
  function getPeriodText(doc){
    var txt='';
    try{
      var top=document.querySelector('#page-report .rpt-topbar span');
      if(top) txt=(top.textContent||'').trim();
    ...
    if(!txt && doc){
      try{
        var spans=Array.prototype.slice.call(doc.querySelectorAll('span'));
        var found=spans.map(function(s){return (s.textContent||'').trim();}).filter(function(t){return /Q[1-4]|20\d{2}|All|الربع|السنة/.test(t) && !/^Target/i.test(t);});
        txt=found.slice(0,3).join(' · ');
      }catch(e){}
    }
    if(!txt){
      try{
        var year=(window.F&&F.year&&F.year!=='all')?F.year:'';
        var q=(window.F&&Array.isArray(F.qtr)&&!F.qtr.includes('all'))?F.qtr.map(function(x){return String(x).toUpperCase();}).join(', '):'';
        txt=[year,q].filter(Boolean).join(' · ');
      }catch(e){}
    }
    return txt;
  }
  function makeMeta(doc,metaOptions){
    ...
    var d;
    try{d=(new Date()).toLocaleString(ar?'ar-SA':'en-GB',{year:'numeric',month:'short',day:'2-digit',hour:'2-digit',minute:'2-digit'});}catch(e){d=(new Date()).toLocaleString();}
    var title=ar?'معلومات الطباعة':'Print Information';
    ...
    html.push('<div><b>'+esc(at)+':</b> '+esc(d)+'</div>');
    if(period) html.push('<div class="qumc-print-meta-period"><b>'+esc(periodLabel)+':</b> '+esc(period)+'</div>');
    var meta=document.createElement('div');
    meta.className='qumc-print-meta';
    meta.dir=ar?'rtl':'ltr';
    meta.innerHTML=html.join('');
    return meta;
  }
  function addPrintCSS(){
    ['qumc-report-print-only-css','qumc-report-print-v4-css','qumc-report-print-v5-css'].forEach(function(id){var o=document.getElementById(id); if(o) o.remove();});
    var css=document.createElement('style');
    ...
    css.textContent=[
      ...
      'body.qumc-print-report-only{background:#fff!important;overflow:visible!important}',
      'body.qumc-print-report-only > :not(#qumcPrintReportPage){display:none!important}',
      '#qumcPrintReportPage{display:block!important;position:static!important;width:auto!important;min-height:auto!important;margin:0!important;padding:0!important;background:#fff!important;color:#152538!important;font-family:Arial,Calibri,sans-serif!important}',
      '#qumcPrintReportPage:before{content:"";position:fixed!important;top:.7mm!important;right:.7mm!important;bottom:.7mm!important;left:.7mm!important;border:2.2px solid #152538!important;border-radius:1.6mm!important;pointer-events:none!important;z-index:999999!important}',
      ...
      '#qumcPrintReportPage .rpt-topbar,#qumcPrintReportPage .rpt-edit-btn,#qumcPrintReportPage button,#qumcPrintReportPage select,#qumcPrintReportPage input{display:none!important}',
      '#qumcPrintReportPage *{box-shadow:none!important}',
      '#qumcPrintReportPage img{max-width:100%!important;print-color-adjust:exact!important;-webkit-print-color-adjust:exact!important}',
      '#qumcPrintReportPage img[alt="QUMC"],#qumcPrintReportPage .qumc-report-header img{height:16mm!important;width:auto!important;max-width:40mm!important;object-fit:contain!important;display:block!important}',
      '#qumcPrintReportPage .qumc-report-header{display:flex!important;align-items:flex-start!important;gap:14px!important;padding:16mm 9mm 8mm!important;background:#0B1C33!important;border-radius:0!important;break-inside:avoid!important;page-break-inside:avoid!important}',
      '#qumcPrintReportPage .qumc-report-header *{visibility:visible!important;opacity:1!important}',
      '#qumcPrintReportPage .qumc-report-header h1{color:#fff!important;font-size:18px!important;line-height:1.25!important;font-weight:900!important;margin:0 0 4px!important}',
      ...
      '#qumcPrintReportPage .qumc-report-header p{color:#00AFCB!important;visibility:visible!important;opacity:1!important}',
      '#qumcPrintReportPage .qumc-report-header span{color:#00AFCB!important;visibility:visible!important;opacity:1!important}',
      '#qumcPrintReportPage .qumc-report-header-line{height:2px!important;background:#00AFCB!important;margin:0 0 6mm!important;break-after:avoid!important}',
      '#qumcPrintReportPage .qumc-print-body{padding:0 5mm 0!important;background:#fff!important}',
      '#qumcPrintReportPage table{width:100%!important;border-collapse:collapse!important;page-break-inside:auto!important}',
      '#qumcPrintReportPage thead{display:table-header-group!important}',
      '#qumcPrintReportPage tr{break-inside:avoid!important;page-break-inside:avoid!important}',
      ...
      '#qumcPrintReportPage h3,#qumcPrintReportPage .qumc-print-section-title{break-after:avoid!important;page-break-after:avoid!important;color:#152538!important;margin-top:3mm!important;margin-bottom:1.8mm!important}',
      '#qumcPrintReportPage p{orphans:3!important;widows:3!important}',
      '#qumcPrintReportPage .rpt-ep{break-inside:avoid!important;page-break-inside:avoid!important;height:auto!important;min-height:0!important;max-height:none!important;overflow:visible!important}',
      '#qumcPrintReportPage .rpt-ep p{line-height:1.55!important;margin:0!important;min-height:0!important}',
      '#qumcPrintReportPage .qumc-print-text-keep,#qumcPrintReportPage .qumc-print-analysis-keep{height:auto!important;min-height:0!important;max-height:none!important;overflow:visible!important;margin-bottom:2mm!important}',
      ...
      '#qumcPrintReportPage .qumc-print-force-next-page{break-before:page!important;page-break-before:always!important;margin-top:0!important}',
      '#qumcPrintReportPage .qumc-print-page-top-space{margin-top:5mm!important}',
      '#qumcPrintReportPage .qumc-print-section-pack.qumc-print-force-next-page{break-before:page!important;page-break-before:always!important}',
      '#qumcPrintReportPage .qumc-print-chart-img{display:block!important;width:100%!important;height:auto!important;max-height:48mm!important;object-fit:contain!important;margin:0 auto!important}',
      '#qumcPrintReportPage .qumc-print-chart-block{break-inside:avoid!important;page-break-inside:avoid!important;min-height:0!important;margin-bottom:2mm!important}',
      ...
      '#qumcPrintReportPage [style*="background:#152538"],#qumcPrintReportPage [style*="background: #152538"],#qumcPrintReportPage [style*="background:#0B1C33"],#qumcPrintReportPage [style*="background:var(--top)"]{background:#0B1C33!important}',
      '#qumcPrintReportPage [style*="color:#0195af"],#qumcPrintReportPage [style*="color: #0195af"]{color:#00AFCB!important}',
      '#qumcPrintReportPage .qumc-print-meta{display:grid!important;grid-template-columns:1fr 1fr!important;gap:3px 16px!important;margin:8mm 5mm 0!important;padding:4mm 5mm!important;border:1px solid #CBD5E1!important;border-radius:4mm!important;background:#F8FAFC!important;color:#152538!important;font:700 9.5px Arial,Calibri,sans-serif!important;line-height:1.55!important;break-inside:avoid!important;page-break-inside:avoid!important}',
  ...
  function cleanup(){
    try{document.body.classList.remove('qumc-print-report-only','qumc-print-report-landscape');}catch(e){}
    try{document.documentElement.classList.remove('qumc-print-report-only','qumc-print-report-landscape');}catch(e){}
    try{var orientationCss=document.getElementById('qumc-report-orientation-css');if(orientationCss)orientationCss.remove();}catch(e){}
    var holder=document.getElementById('qumcPrintReportPage');
    if(holder) holder.remove();
  }
  function convertCanvasesToImages(original, clone){
    var origCanvases=Array.prototype.slice.call(original.querySelectorAll('canvas'));
    var cloneCanvases=Array.prototype.slice.call(clone.querySelectorAll('canvas'));
    origCanvases.forEach(function(oc,i){
      var cc=cloneCanvases[i];
      if(!oc || !cc) return;
      try{
        if(window.CH && oc.id && CH[oc.id] && typeof CH[oc.id].update==='function'){
          CH[oc.id].resize();
          CH[oc.id].update('none');
        }
      }catch(e){}
      try{
        var data=oc.toDataURL('image/png',1);
        if(!data || data==='data:,') return;
        var img=document.createElement('img');
        img.className='qumc-print-chart-img';
        img.src=data;
        img.alt=cc.id || 'Report chart';
        cc.parentNode && cc.parentNode.replaceChild(img,cc);
      }catch(e){ }
    });
  }
  function normalizeClone(clone){
    try{
      ...
      if(first){
        first.classList.add('qumc-report-header');
        var next=first.nextElementSibling;
        if(next && (next.textContent||'').trim()==='') next.classList.add('qumc-report-header-line');
      }
      var bodyCandidate=first;
      while(bodyCandidate && bodyCandidate.nextElementSibling) bodyCandidate=bodyCandidate.nextElementSibling;
      // Put a class on the main body section if the template has a body wrapper after the header line.
      var kids=Array.prototype.slice.call(clone.children);
      kids.forEach(function(k,idx){ if(idx>=2) k.classList.add('qumc-print-body'); });
      clone.querySelectorAll('table').forEach(function(t){(t.parentElement||t).classList.add('qumc-print-table-block');});
      ...
      clone.querySelectorAll('h2,h3').forEach(function(h){
        h.classList.add('qumc-print-section-title');
        if(h.parentElement && h.parentElement.tagName==='DIV') h.parentElement.classList.add('qumc-print-section-heading-wrap');
      });
      // Keep each section title with the first content block after it, so titles do not sit alone
      // at the bottom of a printed page. This grouping is print-clone only and does not touch UI.
      Array.prototype.slice.call(clone.querySelectorAll('.qumc-print-section-heading-wrap')).forEach(function(head){
        try{
      ...
      Array.prototype.slice.call(clone.querySelectorAll('div,p,section,article')).forEach(function(d){
        ...
        if(d.classList && d.classList.contains('rpt-ep')) return;
        if(txt.length && txt.length<1500 && (looksLikeBox || isNarrative)){
          d.classList.add('qumc-print-small-avoid');
  ...
  function fixSoftTextPageBreaks(holder){
    ...
    try{
      if(!holder) return;
      var pagePx=(297-8-7)*(96/25.4);
      var topReserve=8*(96/25.4);
      var bottomReserve=16*(96/25.4);
      var holderTop=holder.getBoundingClientRect().top;
      var items=Array.prototype.slice.call(holder.querySelectorAll('.qumc-print-section-pack,.qumc-print-section-title,.qumc-print-text-keep,.qumc-print-small-avoid,.qumc-print-keep-box,.qumc-print-chart-block,.qumc-print-table-block'));
      items.forEach(function(el){
        try{
          ...
          if(top<0 || h<=0) return;
          var pos=top%pagePx;
          var isTitle=el.classList.contains('qumc-print-section-title') || el.classList.contains('qumc-print-section-pack');
          var maxKeep=pagePx*0.62;
          if(top>pagePx && pos<topReserve){
            el.classList.add('qumc-print-page-top-space');
          }
    ...
    if(reportIsActive()){
      try{
        if(prepare()){
          setTimeout(function(){
            try{nativePrint();}
            finally{setTimeout(cleanup,1600);}
          },260);
          return;
        }
      }catch(e){try{console.warn('QUMC report print failed',e);}catch(_e){}}
    }
    return nativePrint();
  };
  window.addEventListener('afterprint',function(){
...
/* ==========================================================
   QUMC REPORT PRINT FIX V6
   - Supersedes V5 report-print wrapper
   - Page border is aligned to the physical A4 page edges
   - Keeps printed-by / printed-at at the end of the report
   - Temporarily clears document title while printing to reduce browser header text
   NOTE: Chrome URL/date/page-number headers are browser settings and cannot be fully
         removed by page JavaScript when "Headers and footers" is enabled.
   ========================================================== */
(function(){
  if(window.__qumcReportPrintFixV6Applied) return;
  window.__qumcReportPrintFixV6Applied=true;
  ...
  function getReportDocument(){return document.getElementById('rptDocument') || document.getElementById('reportDoc');}
  function reportIsActive(){
    var exportOverride=document.getElementById('kpi-print-override');
    var reportPage=document.getElementById('page-report');
    var doc=getReportDocument();
  ...
  function getPeriodText(doc){
    var txt='';
    try{var top=document.querySelector('#page-report .rpt-topbar span'); if(top) txt=(top.textContent||'').trim();}catch(e){}
    if(!txt && doc){
      try{
        var spans=Array.prototype.slice.call(doc.querySelectorAll('span'));
        var found=spans.map(function(s){return (s.textContent||'').trim();}).filter(function(t){return /Q[1-4]|20\d{2}|All|الربع|السنة/.test(t) && !/^Target/i.test(t);});
  ...
  function addPrintCSS(){
    ['qumc-report-print-only-css','qumc-report-print-v4-css','qumc-report-print-v5-css','qumc-report-print-v6-css'].forEach(function(id){var o=document.getElementById(id); if(o) o.remove();});
    var css=document.createElement('style');
    ...
    css.textContent=[
      ...
      '#qumcPrintReportPage img{max-width:100%!important;print-color-adjust:exact!important;-webkit-print-color-adjust:exact!important}',
      '#qumcPrintReportPage img[alt="QUMC"],#qumcPrintReportPage .qumc-report-header img{height:16mm!important;width:auto!important;max-width:40mm!important;object-fit:contain!important;display:block!important}',
      '#qumcPrintReportPage .qumc-report-header{display:flex!important;align-items:flex-start!important;gap:10px!important;padding:6mm 6mm 4mm!important;background:#0B1C33!important;border-radius:0!important;break-inside:avoid!important;page-break-inside:avoid!important}',
      '#qumcPrintReportPage .qumc-report-header *{visibility:visible!important;opacity:1!important}',
      '#qumcPrintReportPage .qumc-report-header h1{color:#fff!important;font-size:18px!important;line-height:1.25!important;font-weight:900!important;margin:0 0 4px!important}',
      '#qumcPrintReportPage .qumc-report-header h2{color:rgba(255,255,255,.82)!important;font-size:11px!important;margin:0 0 8px!important}',
  ...
  function cleanup(){
    try{document.body.classList.remove('qumc-print-report-only');}catch(e){}
    try{document.documentElement.classList.remove('qumc-print-report-only');}catch(e){}
    try{if(window.__qumcReportPrintTitleWasSet){document.title=window.__qumcReportPrintOriginalTitle||''; window.__qumcReportPrintTitleWasSet=false;}}catch(e){}
    var holder=document.getElementById('qumcPrintReportPage');
    if(holder) holder.remove();
  }
  function convertCanvasesToImages(original, clone){
    ...
    origCanvases.forEach(function(oc,i){
      var cc=cloneCanvases[i];
      if(!oc || !cc) return;
      try{if(window.CH && oc.id && CH[oc.id] && typeof CH[oc.id].update==='function'){CH[oc.id].resize();CH[oc.id].update('none');}}catch(e){}
      try{
        var data=oc.toDataURL('image/png',1);
        if(!data || data==='data:,') return;
        var img=document.createElement('img');
  ...
  function normalizeClone(clone){
    try{
      var first=clone.firstElementChild;
      if(first){first.classList.add('qumc-report-header'); var next=first.nextElementSibling; if(next && (next.textContent||'').trim()==='') next.classList.add('qumc-report-header-line');}
      var kids=Array.prototype.slice.call(clone.children);
      kids.forEach(function(k,idx){ if(idx>=2) k.classList.add('qumc-print-body'); });
      clone.querySelectorAll('table').forEach(function(t){(t.parentElement||t).classList.add('qumc-print-table-block');});
      clone.querySelectorAll('.qumc-print-chart-img,canvas').forEach(function(c){(c.parentElement||c).classList.add('qumc-print-chart-block');});
  ...
  function fixSoftTextPageBreaks(holder){
    ...
    try{
      ...
      var bottomReserve=16*(96/25.4);
      var holderTop=holder.getBoundingClientRect().top;
      var keep=Array.prototype.slice.call(holder.querySelectorAll('.qumc-print-keep-box,.qumc-print-text-keep,.qumc-print-small-avoid,.qumc-print-analysis-keep'));
      ...
      Array.prototype.slice.call(holder.querySelectorAll('.qumc-print-section-title')).forEach(function(el){
        try{
          ...
          var absTop=r.top-holderTop;
          var pos=absTop%pagePx;
          var topReserve=18*(96/25.4);
...
(function(){
  ...
  function addStyle(){
    ...
    st.textContent='@media print{#qumcPrintReportPage .qumc-print-avoid-split-now{break-before:auto!important;page-break-before:auto!important;break-inside:avoid!important;page-break-inside:avoid!important;margin-top:2mm!important}#qumcPrintReportPage .qumc-print-section-title.qumc-print-avoid-split-now{margin-top:3mm!important}#qumcPrintReportPage .qumc-print-analysis-keep,#qumcPrintReportPage .qumc-print-text-keep,#qumcPrintReportPage .qumc-print-keep-box{orphans:2!important;widows:2!important}#qumcPrintReportPage .qumc-print-analysis-keep p,#qumcPrintReportPage .qumc-print-text-keep p{margin-top:0!important;margin-bottom:1.4mm!important}}';
    document.head.appendChild(st);
  }
  function tune(){
    try{
...
/* ==========================================================
   QUMC Report Print V11 — top-safe zone for page-start titles and text boxes
   - Moves only report section titles / narrative boxes that Chrome places
     directly on the printed page frame.
   - Keeps the approved frame position and thickness.
   - Does not resize fonts, tables, charts, or report content.
   ========================================================== */
(function(){
  'use strict';
  if(window.__QUMC_REPORT_PRINT_V11_TOP_SAFE_ZONE__) return;
  window.__QUMC_REPORT_PRINT_V11_TOP_SAFE_ZONE__=true;

  function inject(){
    try{
      var old=document.getElementById('qumc-report-print-v11-top-safe-zone');
      if(old) old.remove();
      var st=document.createElement('style');
      st.id='qumc-report-print-v11-top-safe-zone';
      st.textContent=[
        '@media print{',
        /* Keep the frame exactly where the user approved it. */
        '#qumcPrintReportPage:before{top:0!important;right:0!important;bottom:0!important;left:0!important;border:2.6px solid #152538!important;border-radius:1.2mm!important}',
        /* These classes are added only in the print clone when an item begins too close to a new page top. */
        '#qumcPrintReportPage .qumc-print-v11-page-start-safe{margin-top:9mm!important}',
        '#qumcPrintReportPage .qumc-print-v11-box-page-start-safe{margin-top:10mm!important}',
        '#qumcPrintReportPage .qumc-print-v11-force-next-page{break-before:page!important;page-break-before:always!important;margin-top:10mm!important}',
        '#qumcPrintReportPage .qumc-print-v11-inner-safe{padding-top:5mm!important;-webkit-box-decoration-break:clone!important;box-decoration-break:clone!important}',
        '}'
      ].join('');
      document.head.appendChild(st);
    }catch(e){}
  ...
  function isNarrativeBox(el){
    try{
      if(!el || !el.getAttribute) return false;
      var st=String(el.getAttribute('style')||'').toLowerCase().replace(/\s+/g,'');
      var txt=String(el.textContent||'').replace(/\s+/g,' ').trim();
      if(!txt || txt.length>2400) return false;
      var box = el.classList.contains('qumc-print-safe-box') || el.classList.contains('qumc-print-keep-box') ||
                el.classList.contains('qumc-print-text-keep') || el.classList.contains('qumc-print-small-avoid') ||
                st.indexOf('border:1pxsolidrgba(1,149,175,.18)')>-1 ||
                st.indexOf('border:1pxsolid#e2e8f0')>-1 || st.indexOf('border:1pxsolid#cbd5e1')>-1 ||
                st.indexOf('border:1pxsolid#bfdbfe')>-1 || st.indexOf('background:rgba(1,149,175,.05)')>-1 ||
                st.indexOf('background:#f0fbff')>-1 || st.indexOf('background:#f8fafc')>-1 || st.indexOf('background:#f0fdfa')>-1;
      var narrative=/^(analysis|interpretation|key insights|executive summary|recommendation|conclusion|forward outlook|التحليل|التفسير|الملخص|التوصيات|الخلاصة)\b/i.test(txt) || /performance\s+(remained|was|is|achieved|below|above)/i.test(txt);
      return !!(box && narrative);
    }catch(e){return false;}
  }
  ...
  function tune(){
    try{
      ...
      /* A4 print clone is built in CSS pixels. This matches the rest of the report print wrapper. */
      var pagePx=297*(96/25.4);
      var holderTop=holder.getBoundingClientRect().top;
      var topSafe=14*(96/25.4);      // breathing space under the page frame
      var bottomSafe=24*(96/25.4);   // used to move short boxes before they split

      var headings=Array.prototype.slice.call(holder.querySelectorAll('.qumc-print-section-title,.qumc-print-section-heading-wrap h2,.qumc-print-section-heading-wrap h3'));
      headings.forEach(function(el){
        try{
          if(!isVisible(el)) return;
          if(el.closest && el.closest('.qumc-report-header')) return;
          var r=el.getBoundingClientRect();
          var absTop=r.top-holderTop;
          if(absTop<pagePx*0.35) return; // do not touch the first report header area
          var pos=((absTop%pagePx)+pagePx)%pagePx;
          if(pos<topSafe){
            el.classList.add('qumc-print-v11-page-start-safe');
            el.style.marginTop='9mm';
          }
        }catch(_e){}
      });

      var boxes=Array.prototype.slice.call(holder.querySelectorAll('div,section,article')).filter(isNarrativeBox);
      boxes.forEach(function(el){
        try{
          if(!isVisible(el)) return;
          var r=el.getBoundingClientRect();
          var absTop=r.top-holderTop;
          if(absTop<pagePx*0.35) return;
          var pos=((absTop%pagePx)+pagePx)%pagePx;
          var h=r.height||0;

          el.classList.add('qumc-print-v11-inner-safe');
          el.style.webkitBoxDecorationBreak='clone';
          el.style.boxDecorationBreak='clone';

          /* If the whole narrative box starts too close to the page frame, move it down. */
          if(pos<topSafe){
            el.classList.add('qumc-print-v11-box-page-start-safe');
            el.style.marginTop='10mm';
          }

          /* If a short narrative box would be split and continue at the next page top,
             start it on the next page with a safe top gap. This fixes the visible
             overlap without changing text/chart/table scale. */
          if(h>0 && h<pagePx*0.34 && (pos+h)>(pagePx-bottomSafe)){
            el.classList.add('qumc-print-v11-force-next-page');
  ...
  var oldPrint=window.print;
  if(typeof oldPrint==='function' && !oldPrint.__qumcV11TopSafeWrapped){
    var wrapped=function(){
      inject();
    ...
    wrapped.__qumcV11TopSafeWrapped=true;
    window.print=wrapped;
  }
  var tries=0;
  var t=setInterval(function(){
