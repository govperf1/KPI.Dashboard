...
function renderReportArabic(){
  ...
  const qBullets=[[q1v,'Q1'],[q2v,'Q2'],[q3v,'Q3'],[q4v,'Q4']].filter(([v])=>v!==null).map(([v,q])=>'• <strong>'+q+':</strong> كانت النتيجة <strong style="color:'+(v>=kpiTarget?'#06845A':'#C42B2B')+'">'+v+'%</strong>'+(v<kpiTarget?'، أقل من الهدف '+kpiTarget+'%.':' — محقق الهدف.')).join('<br>');
...
(function(){
  ...
  window.renderReportArabic=function(){
    ...
    var qBul=Object.keys(qvls).filter(function(q){return qvls[q]!==null;}).map(function(q){var v=qvls[q];return '• <b>'+q+':</b> النتيجة <b style="color:'+(v>=target?'#06845A':'#C42B2B')+'">'+v+'%</b>'+(v>=target?' ومحقق الهدف.':' وهي أقل من الهدف '+target+'%.');}).join('<br>')||'لا توجد نتائج ربعية مدخلة للفترة المحددة.';
    var missList=missKs.slice(0,8).map(function(k){var v=qv(k);return '<li><b>'+_esc(k.id)+'</b> — '+_esc(_name(k))+' : '+(v!==null?v.toFixed(1)+'%':'—')+' مقابل الهدف '+k.target+'%</li>';}).join('')||'<li>لا توجد مؤشرات لم يحقق الهدفة ضمن الفلتر الحالي.</li>';
    ...
    el.innerHTML='<div style="padding:0;background:var(--bg);min-height:100%;direction:rtl;text-align:right">'+
      ...
      '<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:9px;margin-bottom:24px">'+[
        ['إجمالي المؤشرات',ks.length,'#EFF6FF','#1E40AF'],['محققة',metKs.length,'#ECFDF5','#06845A'],['لم يحقق الهدفة',missKs.length,missKs.length?'#FEF2F2':'#ECFDF5',missKs.length?'#C42B2B':'#06845A'],['قيد المتابعة',pending.length,'#F8FAFC','#64748B'],['متوسط الأداء',avg!==null?avg+'%':'—',isMet?'#ECFDF5':'#FEF2F2',isMet?'#06845A':'#C42B2B']
      ...
      SH(2,'نظرة عامة على المؤشر')+'<table class="rpt-ar-table"><tr><th>اسم اسم المؤشر</th><td>'+_esc(title)+'</td></tr><tr><th>القسم</th><td>'+_esc(deptFull)+'</td></tr><tr><th>الفترة</th><td>'+period+'</td></tr><tr><th>الهدف</th><td>'+target+'%</td></tr><tr><th>النتيجة</th><td>'+(avg!==null?avg+'%':'غير متاح')+' — '+(avg===null?'قيد المتابعة':isMet?'محقق':'أقل من الهدف')+'</td></tr></table>'+
...
(function(){
  ...
  function addArabicCharts(){
    ...
    setTimeout(function(){try{
      ...
      if(donut)new Chart(donut,{type:'doughnut',data:{labels:['محقق','لم يحقق الهدف','قيد المتابعة'],datasets:[{data:[met,miss,pending],backgroundColor:['#22C55E','#EF4444','#CBD5E1'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom'}}}});
