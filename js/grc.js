...
(function(){
  ...
  window.__QUMC_GRC_BUILD__='20260707-grc-executive-chart-polish-v14';
  ...
  function executiveGovernancePortfolio(){
    var defs=[['policies','policies','policy'],['plans','plans','plan'],['emergencyPlans','emergencyPlans','emergencyPlan'],['forms','forms','form']];
    var cards='<div class="grc-metric-grid cols-4 grc-exec-portfolio-cards">'+defs.map(function(d){var arr=filterDept(state[d[0]],'allFms'),b=governanceBuckets(arr),sub=b.active+' '+L('active')+' · '+b.expired+' '+L('expired');return metricCard(L(d[1]),arr.length,'info',sub,'window._grcOpenMetric(\''+d[0]+'\',\'total\',\'allFms\')');}).join('')+'</div>';
    ...
    var volume=defs.map(function(d,i){return{label:L(d[1]),value:filterDept(state[d[0]],'allFms').length,color:[chartPalette.teal,chartPalette.blue,chartPalette.violet,chartPalette.navy][i]};});
  ...
  function executiveRiskPortfolio(){
    return'<div class="grc-exec-risk-summary">'+riskMetricCards('allFms')+'</div>'+
      ...
      '<div class="grc-exec-operations-grid"><section class="grc-exec-ops-panel"><div class="grc-exec-ops-head"><div><h3>'+L('incidents')+'</h3><p>'+L('incidentTrendChart')+'</p></div><span>'+state.incidents.length+'</span></div>'+incidentMetricCards('allFms')+incidentTrendChartHtml('allFms')+'</section>'+
