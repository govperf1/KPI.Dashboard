...
/* ─────────────────────────────────────────────────────────────
   FINAL FIX — persistent notifications render
   Purpose:
   - Refresh/re-render must never replace history with an empty message.
   - History key is stable by email, not role/department timing.
   - Read notifications stay visible; unread stay on top.
   ───────────────────────────────────────────────────────────── */
(function(){
  ...
  function emailKey(){
    ...
    if(!e){
      try{ e=String(sessionStorage.getItem('qumc_user_email')||localStorage.getItem('qumc_user_email')||'').toLowerCase().trim(); }catch(_){ }
    }
    if(e){try{sessionStorage.setItem('qumc_user_email',e);localStorage.setItem('qumc_user_email',e);}catch(_){}}
    return e || 'guest';
  }
  function hKey(){return 'qumc_notifications_history_stable_'+emailKey();}
  function sKey(){return 'qumc_notifications_seen_stable_'+emailKey();}
  function readJson(key){try{var v=JSON.parse(localStorage.getItem(key)||'[]');return Array.isArray(v)?v:[];}catch(_){return[];}}
  function writeJson(key,val){try{localStorage.setItem(key,JSON.stringify((val||[]).slice(0,150)));}catch(_){}}
  function readSeen(){return readJson(sKey());}
  function writeSeen(ids){writeJson(sKey(),Array.from(new Set(ids||[])));}
  function readLegacyHistory(){
    var out=[];
    try{
      for(var i=0;i<localStorage.length;i++){
        var k=localStorage.key(i)||'';
        if(k.indexOf('qumc_notifications_history_')===0){
          var arr=readJson(k);
          arr.forEach(function(n){if(n&&n.id)out.push(n);});
        }
      }
    }catch(_){ }
    return out;
  ...
  function readHistory(){
    var out=readJson(hKey()).concat(readLegacyHistory());
    var by={};
    out.forEach(function(n){if(n&&n.id&&!by[n.id])by[n.id]=n;});
    return Object.keys(by).map(function(id){return by[id];}).sort(function(a,b){return (b.ts||0)-(a.ts||0);});
  }
  function writeHistory(arr){
    var by={};
    (arr||[]).forEach(function(n){if(n&&n.id)by[n.id]=Object.assign({},by[n.id]||{},n,{ts:n.ts||Date.now()});});
  ...
  function qValues(k){
    var out=[];
    ['q1','q2','q3','q4'].forEach(function(q){var v=k&&k[q]; if(v===undefined&&k)v=k[q.toUpperCase()]; if(v!==undefined&&v!==null&&v!=='')out.push({q:q,v:v});});
    return out;
  }
  function isMet(k,v){
    try{if(typeof window.metStatus==='function')return !!window.metStatus(k,v);}catch(_){ }
    var n=num(v); if(n===null)return true;
    var t=num(k&&((k.target!=null?k.target:k.tg)!=null?(k.target!=null?k.target:k.tg):k.targetValue)); if(t===null)t=100;
    var op=String((k&&k.op)||'>=').toLowerCase();
    if(op.indexOf('<=')>-1||op.indexOf('less')>-1)return n<=t;
    if(op==='='||op.indexOf('equal')>-1)return Math.abs(n-t)<0.0001;
    return n>=t;
  }
  function canSee(k){
    var role=String(window._fbRole||window.currentUserRole||'').toLowerCase();
    if(role.indexOf('super')>-1||role==='admin'||role==='executive')return true;
    var d=String(window._fbDept||window._lockedDept||window.currentUserDept||'').toLowerCase().trim();
    var kd=String(k&&(k.dept||k.department||k.departmentId)||'').toLowerCase().trim();
    return !d || !kd || d===kd;
  }
  function freshNotifications(){
    var ks=[];
    try{ks=(typeof window.allK==='function')?window.allK():[];}catch(_){ks=[];}
    var out=[];
    (ks||[]).forEach(function(k){
      if(!k||!canSee(k))return;
      var bad=qValues(k).filter(function(x){return !isMet(k,x.v);});
      if(!bad.length)return;
      var qs=bad.map(function(x){return String(x.q).toUpperCase();}).join(', ');
      var id='miss_'+kId(k)+'_'+String(k.yr||k.year||'')+'_'+qs;
      out.push({id:id,level:'red',title:(kId(k)||'KPI')+' — '+(k.nameEn||k.nameAr||k.name||''),meta:'Missed target in '+qs,ts:Date.now()});
    });
    return out;
  }
  function mergeHistory(fresh){
    var hist=readHistory();
    if(fresh&&fresh.length) hist=fresh.concat(hist);
    writeHistory(hist);
    return readHistory();
  }
  function getAll(){
    var fresh=freshNotifications();
    if(fresh.length) return mergeHistory(fresh);
    var hist=readHistory();
    if(hist.length) return hist;
    return [];
  }
  function renderFinalNotifications(){
    var list=$('userAlertList'), count=$('userAlertCount');
    var seen=readSeen();
    var fresh=freshNotifications();
    if(fresh.length) mergeHistory(fresh);
    var all=getAll();
    var unreadFresh=fresh.filter(function(n){return seen.indexOf(n.id)<0;});
    if(count){count.textContent=unreadFresh.length;count.style.display=unreadFresh.length?'flex':'none';}
    if(!list)return;
    if(!all.length){
      if(window.__qumcNotifLastHtml){ list.innerHTML=window.__qumcNotifLastHtml; return; }
      list.innerHTML='<div class="qumc-n-empty-final">Loading notifications…</div>';
      return;
    }
    var unread=all.filter(function(n){return seen.indexOf(n.id)<0;});
    var read=all.filter(function(n){return seen.indexOf(n.id)>=0;});
    var rows=unread.concat(read);
    window._notifMap={}; rows.forEach(function(n){window._notifMap[n.id]=n;});
    var html=rows.map(function(n){
      var isRead=seen.indexOf(n.id)>=0;
      var col=isRead?'#555':(n.level==='red'?'#C42B2B':(n.level==='orange'?'#D97706':'#0195af'));
      return '<div class="qumc-nrow-final" data-nid="'+esc(n.id)+'" style="opacity:'+(isRead?'.6':'1')+'">'
        +'<span class="qumc-n-dot-final" style="background:'+col+'"></span>'
        +'<div><div class="qumc-n-title-final">'+esc(n.title)+(isRead?' <span style="font-size:9px;color:#555">(read)</span>':'')+'</div>'
        +'<div class="qumc-n-meta-final">'+esc(n.meta||'')+'</div></div></div>';
    }).join('');
    window.__qumcNotifLastHtml=html;
    list.innerHTML=html;
    Array.prototype.forEach.call(list.querySelectorAll('.qumc-nrow-final'),function(row){
      row.onclick=function(ev){
        ev.preventDefault();ev.stopPropagation();
        var id=row.getAttribute('data-nid'); var s=readSeen(); if(s.indexOf(id)<0){s.push(id);writeSeen(s);} 
        if(typeof window._showNotifModal==='function') window._showNotifModal((window._notifMap||{})[id]);
        setTimeout(renderFinalNotifications,80);
      };
    });
    var clear=$('qumcClearNotifs');
    if(clear){clear.onclick=function(ev){ev.preventDefault();ev.stopPropagation();writeSeen(readSeen().concat(rows.map(function(n){return n.id;})));renderFinalNotifications();};}
  }
  window.renderNotifications=renderFinalNotifications;
  window.updateAlertUI=renderFinalNotifications;
  window.buildUserAlerts=function(){return getAll();};
  window.collectNotifications=function(){return getAll();};
  var oldToggle=window.toggleUserAlerts;
  window.toggleUserAlerts=function(ev){
    if(ev){ev.preventDefault();ev.stopPropagation();if(ev.stopImmediatePropagation)ev.stopImmediatePropagation();}
    var d=$('userAlertDrop'), p=$('userProfileDrop'), b=$('userAlertBtn'); if(!d)return false;
    if(p){p.style.display='none';p.classList.remove('qumc-profile-open','qumc-final-open');}
    var open=d.style.display!=='block';
    if(open){
      try{ if(typeof positionDrop==='function') positionDrop(d,b,320); }catch(_){ }
      if(b){
        try{var r=b.getBoundingClientRect(), w=320; d.style.position='fixed'; d.style.width=w+'px'; d.style.right='auto'; d.style.top=(r.bottom+10)+'px'; d.style.left=Math.max(10,Math.min(window.innerWidth-w-10,r.right-w))+'px'; d.style.zIndex='2147483646';}catch(_e){}
      }
      d.style.display='block';d.classList.add('qumc-final-open','qumc-stay-open');renderFinalNotifications();
    }else{d.style.display='none';d.classList.remove('qumc-final-open','qumc-stay-open');}
    return false;
  };
  function bind(){
    renderFinalNotifications();
    var ab=$('userAlertBtn');
    if(ab&&ab.dataset.qumcPersistentNotif!=='1'){
      ab.dataset.qumcPersistentNotif='1';ab.onclick=null;ab.addEventListener('click',window.toggleUserAlerts,true);
    }
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bind);else bind();
  ...
  setInterval(function(){
    var d=$('userAlertDrop');
    if(d&&d.style.display==='block')renderFinalNotifications();
  },1500);
})();

/* ==========================================================
   FINAL QUMC FIX — stable badge + professional translucent notification modal
   ========================================================== */
(function(){
  function $(id){return document.getElementById(id);} 
  function esc(s){return String(s==null?'':s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');}
  function emailKey(){
    var e=String(window._fbUser||window._fbEmail||window.currentUserEmail||'').toLowerCase().trim();
    if(!e){try{e=String(sessionStorage.getItem('qumc_user_email')||localStorage.getItem('qumc_user_email')||'').toLowerCase().trim();}catch(_){}}
    return e||'guest';
  }
  function readJson(k){try{var a=JSON.parse(localStorage.getItem(k)||'[]');return Array.isArray(a)?a:[];}catch(_){return[];}}
  function writeJson(k,a){try{localStorage.setItem(k,JSON.stringify((a||[]).slice(0,200)));}catch(_){}}
  function hKey(){return 'qumc_notifications_history_stable_'+emailKey();}
  function sKey(){return 'qumc_notifications_seen_stable_'+emailKey();}
  function readSeen(){return readJson(sKey());}
  function writeSeen(a){writeJson(sKey(),Array.from(new Set(a||[])));}
  function readHistory(){
    var out=readJson(hKey()), by={};
    out.forEach(function(n){if(n&&n.id)by[n.id]=n;});
    return Object.keys(by).map(function(id){return by[id];}).sort(function(a,b){return (b.ts||0)-(a.ts||0);});
  }
  function writeHistory(a){
    var by={}; (a||[]).forEach(function(n){if(n&&n.id)by[n.id]=Object.assign({},by[n.id]||{},n,{ts:n.ts||Date.now()});});
    writeJson(hKey(),Object.keys(by).map(function(id){return by[id];}).sort(function(a,b){return (b.ts||0)-(a.ts||0);}));
  }
  function num(v){var n=Number(v);return isFinite(n)?n:null;}
  function kId(k){return String(k&&(k.id||k.code||'')).trim();}
  function qValues(k){var out=[];['q1','q2','q3','q4'].forEach(function(q){var v=k&&k[q];if(v!==undefined&&v!==null&&v!=='')out.push({q:q,v:v});});return out;}
  function isMet(k,v){try{if(typeof window.metStatus==='function')return !!window.metStatus(k,v);}catch(_){ }var n=num(v),t=num(k&&k.target);if(n===null)return true;if(t===null)t=100;var op=String((k&&k.op)||'>=');if(op==='<=')return n<=t;if(op==='=')return Math.abs(n-t)<=0.05;return n>=t;}
  function canSee(k){var role=String(window._fbRole||window.currentUserRole||'').toLowerCase();if(role.indexOf('super')>-1||role==='admin'||role==='executive')return true;var d=String(window._fbDept||window._lockedDept||window.currentUserDept||'').toLowerCase().trim();var kd=String(k&&(k.dept||k.department)||'').toLowerCase().trim();return !d||!kd||d===kd;}
  function fresh(){
    var ks=[];try{ks=(typeof allK==='function')?allK():[];}catch(_){ks=[];}
    var out=[];
    (ks||[]).forEach(function(k){
      if(!k||!canSee(k))return;
      var bad=qValues(k).filter(function(x){return !isMet(k,x.v);}); if(!bad.length)return;
      var qs=bad.map(function(x){return x.q.toUpperCase();}).join(', ');
      out.push({id:'miss_'+kId(k)+'_'+String(k.yr||'')+'_'+qs,level:'red',title:(kId(k)||'KPI')+' — '+(k.nameEn||k.nameAr||''),meta:(lang==='ar'?'لم يحقق الهدف في ':'Missed target in ')+qs,body:(lang==='ar'?'هذا الإشعار مرتبط بمؤشر أقل من الهدف ويحتاج متابعة الإجراء التصحيحي.':'This KPI is below target and requires follow-up on the corrective action.'),ts:Date.now()});
    });
    return out;
  }
  function merge(a){var h=readHistory(); if(a&&a.length)h=a.concat(h); writeHistory(h); return readHistory();}
  function all(){var f=fresh(); if(f.length)return merge(f); return readHistory();}
  window.renderNotifications=function(){
    ...
    if(count){count.textContent=unread.length;count.style.display=unread.length?'flex':'none';}
    if(!list)return;
    if(!rows.length){ if(window.__qumcNotifLastHtml){list.innerHTML=window.__qumcNotifLastHtml;return;} list.innerHTML='<div class="qumc-n-empty-final">'+(lang==='ar'?'لا توجد إشعارات مهمة حالياً.':'No important notifications.')+'</div>'; return; }
    var ordered=unread.concat(rows.filter(function(n){return seen.indexOf(n.id)>=0;}));
    window._notifMap={}; ordered.forEach(function(n){window._notifMap[n.id]=n;});
    var html=ordered.map(function(n){var r=seen.indexOf(n.id)>=0;var col=r?'#94A3B8':(n.level==='red'?'#C42B2B':n.level==='orange'?'#D97706':'#0195af');return '<div class="qumc-nrow-final" data-nid="'+esc(n.id)+'" style="opacity:'+(r?'.64':'1')+'"><span class="qumc-n-dot-final" style="background:'+col+'"></span><div><div class="qumc-n-title-final">'+esc(n.title)+(r?' <span style="font-size:9px;color:#94A3B8">'+(lang==='ar'?'(مقروء)':'(read)')+'</span>':'')+'</div><div class="qumc-n-meta-final">'+esc(n.meta||'')+'</div></div></div>';}).join('');
    window.__qumcNotifLastHtml=html; list.innerHTML=html;
    Array.prototype.forEach.call(list.querySelectorAll('.qumc-nrow-final'),function(row){row.onclick=function(ev){ev.preventDefault();ev.stopPropagation();var id=row.getAttribute('data-nid');var s=readSeen();if(s.indexOf(id)<0){s.push(id);writeSeen(s);} if(window._showNotifModal)window._showNotifModal((window._notifMap||{})[id]); setTimeout(window.renderNotifications,80);};});
    var clear=$('qumcClearNotifs'); if(clear)clear.onclick=function(ev){ev.preventDefault();ev.stopPropagation();writeSeen(readSeen().concat(ordered.map(function(n){return n.id;})));window.renderNotifications();};
  };
  window.updateAlertUI=window.renderNotifications;
  window.buildUserAlerts=function(){return all();};
  window.collectNotifications=function(){return all();};
  window._showNotifModal=function(n){
    if(!n)return; var old=$('_notifModal'); if(old)old.remove(); var isAr=(typeof lang!=='undefined'&&lang==='ar');
    var c=n.level==='red'?'#C42B2B':n.level==='orange'?'#D97706':'#0195af';
    var ov=document.createElement('div');ov.id='_notifModal';ov.style.cssText='position:fixed;inset:0;z-index:2147483647;background:rgba(8,18,35,.42);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:18px;';
    var box=document.createElement('div');box.style.cssText='width:min(460px,94vw);background:rgba(255,255,255,.88);border:1px solid rgba(255,255,255,.65);box-shadow:0 26px 80px rgba(2,8,23,.28);border-radius:22px;padding:0;overflow:hidden;direction:'+(isAr?'rtl':'ltr');
