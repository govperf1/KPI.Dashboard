...
(function(){
  ...
  function requestTypeDashboard(title,type,base){
    var rows=base.filter(function(r){return normalizedType(r)===type;}),open=rows.filter(function(r){return statusKey(r.status)==='open';}).length,progress=rows.filter(function(r){return statusKey(r.status)==='in_progress';}).length,closed=rows.filter(function(r){return statusKey(r.status)==='closed';}).length;
    return'<div class="adv-type-dashboard"><div class="adv-type-dashboard-head"><div><h3>'+title+'</h3><p>'+PLATFORM_LABELS[currentPlatform]+' · Request workflow summary</p></div><span>'+rows.length+' requests</span></div><div class="adv-type-dashboard-metrics">'+dashboardMetric('Total',rows.length,'All requests in this category','info')+dashboardMetric('Open',open,'Newly submitted requests','info')+dashboardMetric('In Progress',progress,'Being handled or awaiting information','warn')+dashboardMetric('Closed',closed,'Closed or cancelled requests','good')+'</div><div class="adv-grid-2">'+departmentBars(rows)+statusSummary(rows)+'</div></div>';
  }
  function dashboardHtml(){
    ...
    return'<section class="adv-view is-active"><div class="grc-section-head"><div><div class="grc-section-title">Review & Development Dashboard</div><div class="grc-section-sub">'+(isAdmin()?'All FMS departments · '+PLATFORM_LABELS[currentPlatform]:'Department scope: '+esc(departmentName(userDepartmentKey()))+' · '+PLATFORM_LABELS[currentPlatform])+'</div></div><span class="grc-section-badge">No personal data</span></div><div class="adv-metric-grid">'+
      metric('Total Requests',base.length,'All submitted requests','all','info')+metric('Existing Item Review & Update',editCount,base.length?Math.round(editCount/base.length*100)+'% of total':'0% of total','edit_review','purple')+metric('New Item Requests',newCount,base.length?Math.round(newCount/base.length*100)+'% of total':'0% of total','new','info')+metric('Closed Requests',approvedCount,'Closed or cancelled requests','approved','good')+metric('Closure Rate',rate+'%',approvedCount+' closed of '+base.length+' requests','approved','good')+
  ...
  function statusSummary(base){function n(s){return base.filter(function(r){return statusKey(r.status)===s;}).length;}return'<div class="adv-card"><h3 class="adv-card-title">Current Status Distribution</h3><div class="adv-status-summary adv-status-summary-3"><div class="adv-status-tile open"><b>'+n('open')+'</b><span>Open</span></div><div class="adv-status-tile in-progress"><b>'+n('in_progress')+'</b><span>In Progress</span></div><div class="adv-status-tile closed"><b>'+n('closed')+'</b><span>Closed</span></div></div></div>';}
