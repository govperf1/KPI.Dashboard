...
(function(){
  ...
  function dashboardHtml(){
    ...
    return'<section class="adv-view is-active"><div class="grc-section-head"><div><div class="grc-section-title">Review & Development Dashboard</div><div class="grc-section-sub">'+(isAdmin()?'All FMS departments · '+PLATFORM_LABELS[currentPlatform]:'Department scope: '+esc(departmentName(userDepartmentKey()))+' · '+PLATFORM_LABELS[currentPlatform])+'</div></div><span class="grc-section-badge">No personal data</span></div><div class="adv-metric-grid">'+
      metric('Total Requests',base.length,'All submitted requests','all','info')+metric('Open Requests',openCount,'Open requests','open','info')+metric('In Progress Requests',progressCount,'Requests currently being handled','in_progress','warn')+metric('Closed Requests',approvedCount,'Closed or cancelled requests','approved','good')+metric('Existing Item Review & Update',editCount,base.length?Math.round(editCount/base.length*100)+'% of total':'0% of total','edit_review','purple')+metric('New Item Requests',newCount,base.length?Math.round(newCount/base.length*100)+'% of total':'0% of total','new','info')+metric('Closure Rate',rate+'%',approvedCount+' closed of '+base.length+' requests','approved','good')+
