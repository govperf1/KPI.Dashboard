/* ======================================================================
   QUMC Review & Guidance Center — Performance + GRC
...
(function(){
  ...
  var TYPE_LABELS={clarification:'Interpretation & Clarification',review:'Professional Review'};
  ...
  function pageSkeleton(platform,rootId){
    ...
    return '<div class="adv-platform-root" id="'+rootId+'" data-adv-platform="'+platform+'"><div class="adv-shell">'+
      '<div class="grc-hero adv-hero"><div class="grc-hero-row"><div><div class="grc-eyebrow">'+platformLabel+' · INTERNAL REVIEW & GUIDANCE SERVICE</div><h1>Review & Guidance Center</h1><p>Submit requests for interpretation, clarification or professional review. Existing edit, approval and ownership workflows remain unchanged.</p></div><div class="grc-hero-actions"><button class="grc-primary-btn" onclick="window._advOpenGuidanceRequest()">＋ New Review & Guidance Request</button></div></div></div>'+
      '<div class="adv-privacy-note"><span class="adv-privacy-icon">i</span><div><strong>Before submitting a request</strong>Service indicators use aggregated non-personal data such as department, request type, category, priority, status and date. Requester identity, request text and attachments are available only to Admin and Super Admin for response and follow-up.</div></div>'+
      '<div class="adv-module-grid">'+
        moduleCard('dashboard','▦','Review & Guidance Dashboard','Aggregated indicators and a non-personal request register scoped to the user\'s department and this platform.')+
        moduleCard('requests','✦','Review & Guidance Requests',isAdmin()?'Review requests, respond, request additional information and close completed cases.':'Submit an interpretation, clarification or professional review request and follow the response workflow.')+
        (isSuperAdmin()?moduleCard('management','⚙','Review & Guidance Center Management','Review request categories and the automatic status workflow.',true):'')+
      '</div><div id="advViewHost"><div class="adv-loading">Loading Review & Guidance Center…</div></div></div></div>';
  }
  ...
  async function loadRecords(){
    ...
    try{
      if(currentView==='dashboard'){
        if(!apiReady('_advisoryGetPublic'))throw new Error('Review and guidance data service is unavailable.');records=await window._advisoryGetPublic();
      }else if(isAdmin()){
        if(!apiReady('_advisoryGetAll'))throw new Error('Review and guidance management service is unavailable.');records=await window._advisoryGetAll();
      }else{
        if(!apiReady('_advisoryGetMine'))throw new Error('Review and guidance request service is unavailable.');records=await window._advisoryGetMine();
      }
  ...
  function renderView(){var h=host();if(!h)return;if(loading){h.innerHTML='<div class="adv-loading">Loading Review & Guidance Center…</div>';return;}if(lastLoadError){h.innerHTML='<div class="adv-error">'+esc(lastLoadError)+'</div>'+(currentView==='dashboard'?dashboardHtml():requestsHtml());return;}if(currentView==='dashboard')h.innerHTML=dashboardHtml();else if(currentView==='requests')h.innerHTML=requestsHtml();else h.innerHTML=managementHtml();}
  ...
  function metric(label,value,sub,filter,tone){
    var icons={info:'▦',good:'✓',warn:'△',bad:'!',purple:'◇',neutral:'•'};
    return'<div class="grc-metric-card adv-metric-card '+tone+' clickable '+(dashboardFilter===filter?'is-active':'')+'" onclick="window._advDashboardFilter(\''+filter+'\')" tabindex="0" role="button">'+
      ...
      '<div class="grc-metric-foot"><span class="grc-metric-sub">'+sub+'</span><span class="grc-metric-arrow">›</span></div></div>';
  }
  function dashboardHtml(){
    ...
    var responded=base.map(responseMinutes).filter(function(x){return x!=null;}),rated=base.filter(function(r){return Number(r.rating)>0;}),sat=rated.length?Math.round(rated.reduce(function(a,r){return a+Number(r.rating);},0)/(rated.length*5)*100):null,displayed=filterRecords(base);
    return '<section class="adv-view is-active"><div class="grc-section-head"><div><div class="grc-section-title">Review & Guidance Dashboard</div><div class="grc-section-sub">'+(isAdmin()?'All FMS Division departments · '+PLATFORM_LABELS[currentPlatform]+' records':'Department scope: '+esc(departmentName(userDepartmentKey()))+' · '+PLATFORM_LABELS[currentPlatform]+' records')+'</div></div><span class="grc-section-badge">No personal data</span></div><div class="adv-metric-grid">'+
      metric('Total Requests',base.length,'Since service launch','all','info')+
      metric('Interpretation & Clarification',clar,base.length?Math.round(clar/base.length*100)+'% of total':'0% of total','clarification','info')+
      metric('Professional Review',rev,base.length?Math.round(rev/base.length*100)+'% of total':'0% of total','review','purple')+
      metric('Open Requests',open,'Under review, in progress or awaiting information','open','warn')+
      metric('Satisfaction Rate',sat==null?'—':sat+'%',rated.length+' rated request'+(rated.length===1?'':'s'),'rated','good')+
  ...
  function requestsHtml(){return'<section class="adv-view is-active"><div class="grc-section-head"><div><div class="grc-section-title">Review & Guidance Requests</div><div class="grc-section-sub">'+(isAdmin()?'Review and respond to '+PLATFORM_LABELS[currentPlatform]+' requests.':'Submit an interpretation, clarification or professional review request and follow the response workflow.')+'</div></div><button class="grc-primary-btn" onclick="window._advOpenGuidanceRequest()">＋ New Review & Guidance Request</button></div>'+(isAdmin()?adminRequestsHtml():ownRequestsHtml())+'</section>';}
  function ownRequestsHtml(){var list=records.slice().sort(function(a,b){return timeMs(b.createdAt)-timeMs(a.createdAt);});return'<div class="adv-action-grid adv-action-grid-single"><button class="adv-action-card" onclick="window._advOpenGuidanceRequest()"><strong>✦ Submit a Review & Guidance Request</strong><span>Choose Interpretation & Clarification or Professional Review. Categories and related records are limited to this platform and your department.</span></button></div><div class="adv-card"><div class="adv-register-toolbar"><div><h3>Submitted Review & Guidance Requests</h3><p>Your '+PLATFORM_LABELS[currentPlatform]+' review and guidance requests only.</p></div><button class="adv-btn ghost" onclick="window._advReload()">Refresh</button></div><div class="adv-table-wrap"><table class="adv-table" style="min-width:980px"><thead><tr><th>Request Code</th><th>Request Type</th><th>Category</th><th>Related Record(s)</th><th>Submitted</th><th>Status</th><th>Last Update</th><th>Rating</th><th></th></tr></thead><tbody>'+(list.length?list.map(function(r){return'<tr><td class="adv-code">'+esc(r.code||r.id)+'</td><td>'+esc(r.requestTypeLabel||TYPE_LABELS[r.requestType]||'—')+'</td><td>'+esc(r.category||'—')+'</td><td>'+esc(relatedText(r))+'</td><td>'+formatDate(r.createdAt,true)+'</td><td>'+statusBadge(r.status)+'</td><td>'+formatDate(r.updatedAt||r.respondedAt||r.createdAt,true)+'</td><td>'+stars(r.rating)+'</td><td><button class="adv-btn secondary" onclick="window._advOpenRequest(\''+esc(r.id)+'\')">Open</button></td></tr>';}).join(''):'<tr><td colspan="9"><div class="adv-empty">No review and guidance requests have been submitted yet.</div></td></tr>')+'</tbody></table></div></div>';}
  ...
  window._advOpenGuidanceRequest=function(){
    closeModal();document.body.classList.add('adv-modal-open');var dept=userDepartmentKey(),ov=document.createElement('div');ov.id='advModal';ov.className='adv-modal-backdrop';ov.innerHTML='<div class="adv-modal"><div class="adv-modal-head"><div><h2>New Review & Guidance Request</h2><p>'+PLATFORM_LABELS[currentPlatform]+' · '+esc(departmentName(dept))+'</p></div><button class="adv-modal-close" onclick="window._advCloseModal()">×</button></div><div class="adv-modal-body"><div class="adv-form">'+
      '<div class="adv-field"><label>Request Type *</label><select id="advRequestType" onchange="window._advTypeChanged()"><option value="">Select request type</option><option value="clarification">Interpretation & Clarification</option><option value="review">Professional Review</option></select></div>'+
      ...
      '<div class="adv-field"><label>Priority</label><select id="advPriority"><option>Low</option><option selected>Normal</option><option>High</option></select></div>'+
  ...
  window._advSubmitConsultation=async function(){
    var type=String((document.getElementById('advRequestType')||{}).value||''),category=String((document.getElementById('advCategory')||{}).value||''),other=String((document.getElementById('advOtherCategory')||{}).value||'').trim(),title=String((document.getElementById('advTitle')||{}).value||'').trim(),details=String((document.getElementById('advDetails')||{}).value||'').trim(),priority=String((document.getElementById('advPriority')||{}).value||'Normal'),file=(document.getElementById('advAttachment')||{}).files&&document.getElementById('advAttachment').files[0],relatedItems=selectedRelatedItems(),relatedNewText=String((document.getElementById('advRelatedNewText')||{}).value||'').trim(),relatedNew=!!((document.getElementById('advRelatedNew')||{}).checked),relatedType=String((document.getElementById('advRelatedType')||{}).value||'');
    if(!type||!category||!title||!details){toast('Complete the request type, category, title and details.');return;}if(category==='Other'&&!other){toast('Enter the other category.');return;}var cfg=relatedConfig(category);if(cfg&&!relatedItems.length&&!relatedNew){toast('Select at least one related record or choose New / Not Listed.');return;}if(relatedNew&&!relatedNewText){toast('Enter the new or unlisted record name.');return;}if(file&&file.size>5*1024*1024){toast('The attachment must be 5 MB or smaller.');return;}if(!apiReady('_advisorySubmit')){toast('Review and guidance request service is unavailable.');return;}
    var dept=userDepartmentKey();try{var result=await window._advisorySubmit({platform:currentPlatform,departmentKey:dept,departmentCode:departmentCode(dept),priority:priority,serviceType:'consultation',requestType:type,requestTypeLabel:TYPE_LABELS[type],category:category==='Other'?other:category,title:title,details:details,relatedType:relatedType,relatedItems:relatedItems,relatedNewText:relatedNew?relatedNewText:''},file||null);audit('GUIDANCE_REQUEST_SUBMIT','Submitted '+currentPlatform+' request '+(result&&result.code||''));closeModal();currentView='requests';await loadRecords(true);toast('Request submitted successfully: '+(result&&result.code||''));}catch(e){toast(String(e&&e.message||e));}
  };
  ...
  function showRequestModal(r){closeModal(false);document.body.classList.add('adv-modal-open');selectedRequest=r;var ov=document.createElement('div');ov.id='advModal';ov.className='adv-modal-backdrop';ov.innerHTML='<div class="adv-modal adv-modal-wide"><div class="adv-modal-head"><div><h2>'+esc(r.code||'Review & Guidance Request')+'</h2><p>'+esc(PLATFORM_LABELS[recordPlatform(r)]+' · '+TYPE_LABELS[r.requestType])+' · '+statusLabel(r.status)+'</p></div><button class="adv-modal-close" onclick="window._advCloseModal()">×</button></div><div class="adv-modal-body">'+requestDetails(r)+(isAdmin()?adminActions(r):requesterActions(r))+'</div></div>';(root()||document.body).appendChild(ov);ov.addEventListener('click',function(e){if(e.target===ov)closeModal();});}
  ...
  window._advSendClarification=async function(){var text=String((document.getElementById('advClarification')||{}).value||'').trim(),file=(document.getElementById('advClarificationFile')||{}).files&&document.getElementById('advClarificationFile').files[0];if(!text){toast('Enter the clarification first.');return;}try{await window._advisoryRequesterAction(selectedRequest.id,'clarify',{text:text},file||null);audit('GUIDANCE_REQUEST_CLARIFICATION','Added clarification to '+selectedRequest.code);closeModal();await loadRecords(true);toast('Clarification sent. The request is now In Progress.');}catch(e){toast(String(e&&e.message||e));}};
  window._advConfirmComplete=async function(){try{await window._advisoryRequesterAction(selectedRequest.id,'complete',{},null);audit('GUIDANCE_REQUEST_COMPLETED','Confirmed completion of '+selectedRequest.code);closeModal();await loadRecords(true);toast('Request marked as Completed.');}catch(e){toast(String(e&&e.message||e));}};
  window._advCancelRequest=async function(){if(!window.confirm('Cancel this request?'))return;try{await window._advisoryRequesterAction(selectedRequest.id,'cancel',{},null);audit('GUIDANCE_REQUEST_CANCELLED','Cancelled '+selectedRequest.code);closeModal();await loadRecords(true);toast('Request cancelled.');}catch(e){toast(String(e&&e.message||e));}};
  window._advRate=async function(rating){try{await window._advisoryRate(selectedRequest.id,rating);audit('GUIDANCE_REQUEST_RATED','Rated '+selectedRequest.code+' '+rating+'/5');closeModal();await loadRecords(true);toast('Thank you. Your satisfaction rating was saved.');}catch(e){toast(String(e&&e.message||e));}};
