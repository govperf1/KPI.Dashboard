...
(function(){
  ...
  function mount(platform,rootId){if(liveUnsub){try{liveUnsub();}catch(_){}liveUnsub=null;}livePayload=null;currentPlatform=platform;currentRootId=rootId;/* Action queues are the operational landing view for Admin/Super Admin. The analytics dashboard remains available as a separate card. */currentView='requests';dashboardFilter='all';departmentFilter='';loading=true;lastLoadError='';renderView();if(isDepartmentManager()){/* Manager approval reads must use the fresh Firestore profile resolved by _advisoryGetManagerQueue, not a stale session listener. */loading=false;loadRecords();var managerTimer=setInterval(function(){if(root()&&currentView==='requests')loadRecords();},5000);liveUnsub=function(){clearInterval(managerTimer);};}else if(apiReady('_advisorySubscribe')){liveUnsub=window._advisorySubscribe(applyLivePayload);}else{loading=false;loadRecords();}}
