...
(function(){
  ...
  function canSeeRequest(r){
    ...
    if(!r)return false;
    if(isSuper())return st==='pending_super_admin'||st==='approved'||st==='rejected_super_admin'||st==='returned_super_admin';
  ...
  window._showGapApprovals=function(){
    ...
    ov.innerHTML='<div style="width:min(860px,96vw);max-height:86vh;overflow:auto;background:linear-gradient(135deg,rgba(255,255,255,.92),rgba(248,250,252,.84));border:1px solid rgba(255,255,255,.72);border-radius:22px;box-shadow:0 28px 80px rgba(15,23,42,.28);padding:22px">'
      ...
      +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><div><div style="font-size:16px;font-weight:900;color:#0f172a">'+(isAr()?'صندوق موافقات تحليل الفجوات':'Gap Analysis Approval Inbox')+'</div><div style="font-size:11px;color:#64748b;margin-top:4px">'+(isAr()?'طلبات الإدخال التي تحتاج مراجعة حسب صلاحيتك':'Approval requests based on your permission scope')+'</div></div><button onclick="document.getElementById(\'_gapApprovalsOv\').remove()" style="width:34px;height:34px;border:1px solid rgba(148,163,184,.25);background:rgba(255,255,255,.75);border-radius:10px;cursor:pointer;color:#334155;font-size:16px">×</button></div>'
  ...
  function injectApprovalButton(){
    ...
    btn.innerHTML='✅ '+(isAr()?'موافقات تحليل الفجوات':'Gap Approval Inbox');
...
(function(){
  ...
  function pendingForApproval(){return visibleReqs().filter(function(r){if(isSuper())return r.status==='pending_super_admin';if(isManager())return r.status==='pending_manager';return /^(pending_manager|pending_super_admin)$/.test(String(r.status||''));});}
  ...
  window._showGapApprovals=function(focusId){
    ...
    var title=isOwner()?(ar?'حالة تحليل الفجوات الخاصة بك':'Your Gap Analysis Status'):(ar?'موافقات تحليل الفجوات':'Gap Analysis Approvals Status');
    ...
    var ownerSummary=isOwner()?statusSummaryHtml(miss.length,ar?'حالة إدخال بيانات الفجوات':'Your gap analysis status',ar?'جميع بيانات الفجوات مكتملة':'All gap analysis data is up to date'):'';
...
(function(){
  ...
  window.showKpoGapStatusPopup=function(){
    ...
    if(isSuper()||isManager()){
      ...
      if(title){title.textContent=ar?'حالة طلبات الاعتماد':'Approval Requests Status';title.setAttribute('data-en','Approval Requests Status');title.setAttribute('data-ar','حالة طلبات الاعتماد');}
    ...
    if(isOwner()){
      ...
      if(title){title.textContent=ar?'حالة تحليل الفجوات الخاصة بك':'Your Gap Analysis Status';title.setAttribute('data-en','Your Gap Analysis Status');title.setAttribute('data-ar','حالة تحليل الفجوات الخاصة بك');}
      html+=statusBodyCard(miss.length,ar?'حالة إدخال بيانات الفجوات':'Your gap analysis status',ar?'جميع بيانات الفجوات مكتملة':'All gap analysis data is up to date');
...
/* ==========================================================
   QUMC GAP APPROVAL V5 — stable reject comment + clean role popup
   - Reject comment appears inside same card and stays visible.
   - Reject comment is mandatory.
   - Removes KPI Owner inline View banner.
   - Stabilizes Approval Requests Status content by using one final renderer.
   ========================================================== */
(function(){
  'use strict';
  if(window.__QUMC_GAP_APPROVAL_V5__) return;
  window.__QUMC_GAP_APPROVAL_V5__=true;
  function $(id){return document.getElementById(id);} 
  function esc(v){return String(v==null?'':v).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}
  function ar(){return (window.lang==='ar'||document.documentElement.dir==='rtl');}
  function role(){return String(window._fbRole||window.currentUserRole||'').toLowerCase().replace(/[\s-]+/g,'_');}
  function email(){return String(window._fbUser||window._fbEmail||window.currentUserEmail||'').toLowerCase().trim();}
  function uname(){return String(window._fbName||window.currentUserName||(email()?email().split('@')[0]:'User'));}
  function deptAlias(v){var x=String(v||'').toLowerCase().replace(/[^a-z0-9\u0600-\u06ff]+/g,'');if(!x)return'';if(x.indexOf('maintenance')>-1||x.indexOf('صيانة')>-1)return'maintenance';if(x.indexOf('safety')>-1||x.indexOf('سلامة')>-1)return'safety';if(x.indexOf('housekeeping')>-1||x.indexOf('cleaning')>-1||x.indexOf('hospitality')>-1||x.indexOf('نظافة')>-1||x.indexOf('فندقة')>-1)return'housekeeping';if(x.indexOf('project')>-1||x.indexOf('مشاريع')>-1||x.indexOf('المشاريع')>-1)return'projects';if(x.indexOf('governance')>-1||x.indexOf('حوكمة')>-1)return'governance';return x;}
  function dept(){return deptAlias(window._fbDept||window._lockedDept||window.currentUserDept||'');}
  ...
  function nkey(v){return String(v||'').toLowerCase().replace(/[^a-z0-9\u0600-\u06ff]+/g,'').trim();}
  function kName(k){return ar()?(k.nameAr||k.nameEn||k.name||k.id):(k.nameEn||k.name||k.nameAr||k.id);}
  function qLabel(q){return String(q||'').toUpperCase();}
  function num(v){if(v===null||v===undefined||v==='')return null;var s=String(v).replace(/[٪%]/g,'').replace(/,/g,'').replace(/[٠-٩]/g,function(c){return '٠١٢٣٤٥٦٧٨٩'.indexOf(c);}).replace(/[۰-۹]/g,function(c){return '۰۱۲۳۴۵۶۷۸۹'.indexOf(c);});var n=Number(s);return isFinite(n)?n:null;}
  function qVal(k,q){var v=k&&k[q];if(v===undefined)v=k&&k[String(q).toUpperCase()];return num(v);}
  function met(k,v){try{if(typeof window.metStatus==='function')return window.metStatus(k,v);}catch(_){}var t=num(k&&k.target);if(v===null)return null;if(t===null)t=100;var op=String(k&&k.op||'>=');if(op==='<=')return v<=t;if(op==='=')return Math.abs(v-t)<=0.05;return v>=t;}
  function gapTexts(o){o=o||{};return{root:String(o.gapEn||o.gapAr||o.rootCause||o.rootCauseEn||o.root||o.reason||o.gapReasons||'').trim(),action:String(o.actEn||o.actAr||o.correctiveAction||o.correctiveActions||o.actionPlan||o.action||o.actions||'').trim(),impact:String(o.impactEn||o.impactAr||o.impact||o.impactOfGap||o.gapImpact||'').trim()};}
  function gapComplete(k,q){var st=window.ST||{},g=st.gaps||{},a=st.actions||{},id=String(k.id||k.kpiCode||'');var keys=[id+'_'+String(q).toLowerCase(),id+'_'+String(q).toUpperCase(),id];for(var i=0;i<keys.length;i++){var gt=gapTexts(g[keys[i]]||{}),at=gapTexts(a[keys[i]]||{});if((gt.root||at.root)&&(gt.action||at.action)&&(gt.impact||at.impact))return true;}return false;}
  function latestReq(k,q,fn){var id=String(k.id||k.kpiCode||''),qq=String(q||'').toLowerCase();var arr=approvals().filter(function(r){return r&&String(r.kpiId||r.kpiCode||'')===id&&String(r.quarter||'').toLowerCase()===qq&&(!fn||fn(r));});arr.sort(function(a,b){return String(b.updatedAt||b.submittedAt||'').localeCompare(String(a.updatedAt||a.submittedAt||''));});return arr[0]||null;}
  function liveApproval(k,q){return latestReq(k,q,function(r){return /^(pending_manager|pending_super_admin|approved)$/.test(String(r.status||''));});}
  function ownLatest(k,q){var me=email();return latestReq(k,q,function(r){return String(r.submittedByEmail||'').toLowerCase()===me;});}
  function canAccessKpi(k){if(isSuper())return true;var kd=deptAlias(k.dept||k.department||'');if(isManager())return kd&&kd===dept();if(isOwner()){var a=window._fbAssignedKpis;if(Array.isArray(a)&&a.length){var vals=a.map(nkey), code=nkey(k.id||k.kpiCode), name=nkey(k.nameEn||k.name||k.nameAr);return vals.indexOf(code)>-1||vals.indexOf(name)>-1;}return kd&&kd===dept();}return false;}
  function ownerItems(){var rows=[];allKpis().forEach(function(k){if(!canAccessKpi(k))return;['q1','q2','q3','q4'].forEach(function(q){var v=qVal(k,q);if(v===null||met(k,v)!==false)return;if(gapComplete(k,q))return;var req=ownLatest(k,q);if(liveApproval(k,q))return;rows.push({k:k,q:q,value:v,target:num(k.target),year:k.yr||k.year||'',dept:k.dept||k.department||'',req:req,rejected:req&&String(req.status||'').indexOf('rejected')===0});});});rows.sort(function(a,b){return (Number(!!b.rejected)-Number(!!a.rejected))||String(a.dept).localeCompare(String(b.dept))||String(a.k.id).localeCompare(String(b.k.id))||a.q.localeCompare(b.q);});return rows;}
  function approvalItems(){return approvals().filter(function(r){if(!r)return false;var st=String(r.status||'');if(isSuper())return st==='pending_super_admin';if(isManager())return st==='pending_manager'&&deptAlias(r.dept)===dept();return false;}).sort(function(a,b){return String(b.updatedAt||b.submittedAt||'').localeCompare(String(a.updatedAt||a.submittedAt||''));});}
  function statusText(st){var A=ar();return ({pending_manager:A?'بانتظار مدير القسم':'Pending Department Manager',pending_super_admin:A?'بانتظار السوبر أدمن':'Pending Super Admin',approved:A?'معتمد':'Approved',rejected_manager:A?'مرفوض من مدير القسم':'Rejected by Department Manager',rejected_super_admin:A?'مرفوض من السوبر أدمن':'Rejected by Super Admin'})[st]||st||'—';}
  function summary(count,label,ok){var red=count>0;return '<div style="margin-bottom:14px;padding:12px 14px;border-radius:13px;background:'+(red?'rgba(220,38,38,.06);border:1px solid rgba(220,38,38,.20)':'rgba(22,163,74,.06);border:1px solid rgba(22,163,74,.20)')+'"><div style="font-size:11px;color:#64748b;font-weight:900;text-transform:uppercase;letter-spacing:.05em">'+esc(label)+'</div><strong style="display:block;margin-top:5px;color:'+(red?'#DC2626':'#16A34A')+';font-size:13px">'+(red?count+' '+(ar()?'عنصر يحتاج إجراء':'item(s) need action'):esc(ok))+'</strong></div>';}
  function actionBtns(r){
    if(isManager()&&String(r.status)==='pending_manager'&&deptAlias(r.dept)===dept())return '<div class="gap-apr-actions" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"><button type="button" onclick="window._gapApprovalAct&&window._gapApprovalAct(\''+esc(r.id)+'\',\'manager_approve\');setTimeout(window.showKpoGapStatusPopup,220)" class="gap-apr-btn ok">Approve</button><button type="button" onclick="window._gapApprovalRejectStart&&window._gapApprovalRejectStart(\''+esc(r.id)+'\',\'manager_reject\')" class="gap-apr-btn bad">Reject</button></div>';
    if(isSuper()&&String(r.status)==='pending_super_admin')return '<div class="gap-apr-actions" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"><button type="button" onclick="window._gapApprovalAct&&window._gapApprovalAct(\''+esc(r.id)+'\',\'super_approve\');setTimeout(window.showKpoGapStatusPopup,220)" class="gap-apr-btn ok">Final Approve</button><button type="button" onclick="window._gapApprovalRejectStart&&window._gapApprovalRejectStart(\''+esc(r.id)+'\',\'super_reject\')" class="gap-apr-btn bad">Reject</button></div>';
    return '';
  }
  function approvalCard(r){var p=r.payload||{};return '<div class="gap-apr-card" id="gap_apr_'+esc(r.id)+'" data-req-id="'+esc(r.id)+'" style="border:1px solid rgba(148,163,184,.25);border-radius:13px;padding:12px;background:rgba(255,255,255,.82);margin-bottom:10px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start"><div><div style="font-size:12px;font-weight:900;color:#0f172a">'+esc(r.kpiCode||r.kpiId||'KPI')+' — '+esc(r.kpiNameEn||r.kpiNameAr||'Gap Analysis')+' · '+esc(qLabel(r.quarter))+'</div><div style="font-size:10px;color:#64748b;margin-top:4px">'+esc(r.dept||'')+' · '+esc(r.year||'')+' · '+esc(r.submittedByName||'')+'</div></div><span style="font-size:10px;font-weight:900;padding:5px 10px;border-radius:999px;color:#0369A1;background:rgba(3,105,161,.10);white-space:nowrap">'+esc(statusText(r.status))+'</span></div><div style="display:grid;grid-template-columns:1fr 1fr;gap:9px;margin-top:10px"><div><b style="font-size:10px;color:#B91C1C">Root Cause</b><div style="font-size:11px;color:#334155;line-height:1.55;margin-top:4px">'+esc(p.gapEn||'—')+'</div></div><div><b style="font-size:10px;color:#047857">Corrective Action</b><div style="font-size:11px;color:#334155;line-height:1.55;margin-top:4px">'+esc(p.actEn||'—')+'</div></div><div><b style="font-size:10px;color:#0369A1">Impact</b><div style="font-size:11px;color:#334155;line-height:1.55;margin-top:4px">'+esc(p.impactEn||'—')+'</div></div><div><b style="font-size:10px;color:#64748b">Accountability</b><div style="font-size:11px;color:#334155;line-height:1.55;margin-top:4px">'+esc(p.owner||'—')+' · '+esc(p.dueDate||'—')+' · '+esc(p.priority||'—')+' · '+esc(p.status||'—')+'</div></div></div>'+actionBtns(r)+'</div>';}
  function ownerCard(x){var note=x.rejected?('<div style="font-size:10px;color:#991B1B;margin-top:5px;font-weight:800">'+(ar()?'تم رفض الطلب السابق — عدلي البيانات وأعيدي الإرسال':'Previous request rejected — edit and resubmit')+(x.req&&(x.req.managerNote||x.req.superAdminNote)?': '+esc(x.req.superAdminNote||x.req.managerNote):'')+'</div>'):'';return '<div class="gap-missing-card" style="border:1px solid '+(x.rejected?'rgba(220,38,38,.24)':'rgba(217,119,6,.18)')+';border-radius:13px;padding:12px;background:'+(x.rejected?'rgba(254,242,242,.76)':'rgba(255,251,235,.72)')+';margin-bottom:10px"><div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start"><div><div style="font-size:12px;font-weight:900;color:#78350f">'+esc(x.k.id)+' — '+esc(kName(x.k))+'</div><div style="font-size:10px;color:#64748b;margin-top:4px">'+esc(x.dept||'')+' · '+esc(x.year||'')+' · '+qLabel(x.q)+' · Result '+esc(x.value)+'% / Target '+esc(x.target)+'%</div>'+note+'</div><button type="button" onclick="closOv&&closOv(\'_kpoStatusOv\');window.openGapQuarter&&window.openGapQuarter(\''+esc(x.k.id)+'\',\''+esc(x.q)+'\')" class="gap-apr-btn warn">'+(x.rejected?(ar()?'تعديل وإعادة الإرسال':'Edit & Resubmit'):(ar()?'إدخال البيانات':'Enter Gap Data'))+'</button></div></div>';}
  function injectStyle(){if($('qumc-approval-v5-style'))return;var st=document.createElement('style');st.id='qumc-approval-v5-style';st.textContent='#qumcApprovalStatusBanner{display:none!important}.gap-apr-btn{border:none;border-radius:10px;padding:8px 14px;font-size:11px;font-weight:900;cursor:pointer}.gap-apr-btn.ok{background:#047857;color:#fff}.gap-apr-btn.warn{background:#D97706;color:#fff}.gap-apr-btn.bad{background:#B91C1C;color:#fff}.qumc-reject-inline{margin-top:12px;background:#fff7ed!important;border:1px solid rgba(217,119,6,.42)!important;border-radius:13px!important;padding:12px!important;box-shadow:inset 0 0 0 1px rgba(255,255,255,.55)!important}.qumc-reject-inline textarea:focus{outline:2px solid rgba(217,119,6,.28)!important;border-color:#D97706!important}';document.head.appendChild(st);} injectStyle();
  window._gapApprovalRejectStart=function(reqId,action){
    injectStyle();var card=$('gap_apr_'+reqId);if(!card){try{window._showGapApprovals&&window._showGapApprovals(reqId);}catch(_){}setTimeout(function(){window._gapApprovalRejectStart(reqId,action);},120);return false;}
    ...
    var box=document.createElement('div');box.className='qumc-reject-inline';
    box.innerHTML='<div style="font-size:11px;font-weight:900;color:#92400e;margin-bottom:7px">'+(ar()?'سبب الرفض مطلوب':'Reject comment is required')+'</div><textarea id="gapRejectComment_'+esc(reqId)+'" style="width:100%;min-height:82px;border:1px solid rgba(217,119,6,.35);border-radius:10px;padding:9px;font-family:inherit;font-size:11.5px;resize:vertical;background:#fff" placeholder="'+(ar()?'اكتب سبب الرفض هنا...':'Write the rejection reason here...')+'"></textarea><div id="gapRejectWarn_'+esc(reqId)+'" style="display:none;margin-top:7px;color:#b45309;font-size:10.5px;font-weight:800">'+(ar()?'لازم كتابة سبب الرفض قبل الإرسال.':'A rejection comment is required before submitting.')+'</div><div style="display:flex;gap:8px;justify-content:flex-end;margin-top:9px"><button type="button" onclick="this.closest(\'.qumc-reject-inline\').remove()" class="gap-apr-btn" style="background:#e2e8f0;color:#334155">'+(ar()?'إلغاء':'Cancel')+'</button><button type="button" onclick="window._gapApprovalAct(\''+esc(reqId)+'\',\''+esc(action)+'\',document.getElementById(\'gapRejectComment_'+esc(reqId)+'\').value)" class="gap-apr-btn bad">'+(ar()?'تأكيد الرفض':'Confirm Reject')+'</button></div>';
    var actions=card.querySelector('.gap-apr-actions');(actions||card).insertAdjacentElement(actions?'afterend':'beforeend',box);box.scrollIntoView({block:'nearest'});var ta=box.querySelector('textarea');if(ta)ta.focus();return false;
  };
  var prevAct=window._gapApprovalAct;
  function nowIso(){return new Date().toISOString();}
  function save(tag,msg){try{if(typeof window.sLS==='function')window.sLS(ST);else localStorage.setItem('kpi_v3',JSON.stringify(ST));}catch(_){}try{if(typeof window._saveToFS==='function')window._saveToFS(ST);}catch(_){}try{if(typeof window.addAudit==='function')window.addAudit(tag,msg||tag);}catch(_){}try{if(typeof window.renderNotifications==='function')setTimeout(window.renderNotifications,120);}catch(_){}}
  function findReq(id){return approvals().find(function(r){return r&&String(r.id)===String(id);});}
  window._gapApprovalAct=function(reqId,action,comment){
    action=String(action||'');
    if(action.indexOf('reject')===-1){return typeof prevAct==='function'?prevAct.apply(this,arguments):false;}
    var note=String(comment||'').trim();
    if(!note){window._gapApprovalRejectStart(reqId,action);var w=$('gapRejectWarn_'+reqId);if(w)w.style.display='block';return false;}
    var r=findReq(reqId); if(!r)return false;
    if(action==='manager_reject'){
      if(!isManager()||deptAlias(r.dept)!==dept()){try{toast('Access denied');}catch(_){}return false;}
      r.status='rejected_manager';r.managerBy=uname();r.managerEmail=email();r.managerAt=nowIso();r.managerNote=note;r.updatedAt=nowIso();(r.history||(r.history=[])).push({at:nowIso(),by:uname(),role:role(),action:'manager_rejected',note:note});save('GAP_APPROVAL_MANAGER_REJECT','Department Manager rejected Gap Analysis: '+(r.kpiCode||r.kpiId||'KPI'));
    }else if(action==='super_reject'){
      if(!isSuper()){try{toast('Access denied');}catch(_){}return false;}
      r.status='rejected_super_admin';r.superAdminBy=uname();r.superAdminEmail=email();r.superAdminAt=nowIso();r.superAdminNote=note;r.updatedAt=nowIso();(r.history||(r.history=[])).push({at:nowIso(),by:uname(),role:role(),action:'super_admin_rejected',note:note});save('GAP_APPROVAL_SUPER_REJECT','Super Admin rejected Gap Analysis: '+(r.kpiCode||r.kpiId||'KPI'));
    }
    try{toast(ar()?'تم رفض الطلب وإرسال سبب الرفض':'Request rejected with comment');}catch(_){}
    setTimeout(function(){try{window.showKpoGapStatusPopup&&window.showKpoGapStatusPopup();}catch(_){}try{window.renderNotifications&&window.renderNotifications();}catch(_){}},220);
    return false;
  };
  window.showKpoGapStatusPopup=function(){
    injectStyle();var ov=$('_kpoStatusOv'), body=$('_kpoStatusBody');if(!ov||!body)return;var A=ar();var title=ov.querySelector('.mhd-t');var btn=ov.querySelector('.af-btn-primary');if(btn)btn.textContent=A?'حسناً':'Got it';var html='<style>.gap-apr-btn{border:none;border-radius:10px;padding:8px 14px;font-size:11px;font-weight:900;cursor:pointer}.gap-apr-btn.ok{background:#047857;color:#fff}.gap-apr-btn.warn{background:#D97706;color:#fff}.gap-apr-btn.bad{background:#B91C1C;color:#fff}</style>';
    if(isSuper()||isManager()){
      var reqs=approvalItems();if(title){title.textContent=A?'حالة طلبات الاعتماد':'Approval Requests Status';title.setAttribute('data-en','Approval Requests Status');title.setAttribute('data-ar','حالة طلبات الاعتماد');}
      html+=summary(reqs.length,A?'طلبات تحتاج موافقة':'Approval requests status',A?'لا توجد طلبات موافقة معلقة':'No pending approval requests');
    ...
    if(isOwner()){
      var miss=ownerItems();if(title){title.textContent=A?'حالة تحليل الفجوات الخاصة بك':'Your Gap Analysis Status';title.setAttribute('data-en','Your Gap Analysis Status');title.setAttribute('data-ar','حالة تحليل الفجوات الخاصة بك');}
      html+=summary(miss.length,A?'حالة إدخال بيانات الفجوات':'Your gap analysis status',A?'جميع بيانات الفجوات مكتملة':'All gap analysis data is up to date');
  ...
  window._showGapApprovals=function(focusId){
    var old=$('_gapApprovalsOv');if(old)old.remove();var A=ar();var reqs=(isSuper()||isManager())?approvalItems():[];var miss=isOwner()?ownerItems():[];var title=isOwner()?(A?'حالة تحليل الفجوات الخاصة بك':'Your Gap Analysis Status'):(A?'حالة طلبات الاعتماد':'Approval Requests Status');
    var html='<style>.gap-apr-btn{border:none;border-radius:10px;padding:8px 14px;font-size:11px;font-weight:900;cursor:pointer}.gap-apr-btn.ok{background:#047857;color:#fff}.gap-apr-btn.warn{background:#D97706;color:#fff}.gap-apr-btn.bad{background:#B91C1C;color:#fff}</style>';
    if(isSuper()||isManager()){html+=summary(reqs.length,A?'طلبات تحتاج موافقة':'Approval requests status',A?'لا توجد طلبات موافقة معلقة':'No pending approval requests')+(reqs.length?reqs.map(approvalCard).join(''):'<div style="padding:22px;text-align:center;color:#047857;font-size:12px;border:1px dashed rgba(22,163,74,.30);border-radius:14px;background:rgba(22,163,74,.06)">'+(A?'لا توجد طلبات تحتاج اعتماد حالياً.':'No requests need your approval right now.')+'</div>');}
    else if(isOwner()){html+=summary(miss.length,A?'حالة إدخال بيانات الفجوات':'Your gap analysis status',A?'جميع بيانات الفجوات مكتملة':'All gap analysis data is up to date')+(miss.length?miss.map(ownerCard).join(''):'<div style="padding:22px;text-align:center;color:#047857;font-size:12px;border:1px dashed rgba(22,163,74,.30);border-radius:14px;background:rgba(22,163,74,.06)">'+(A?'لا توجد بيانات فجوات ناقصة ضمن صلاحيتك.':'No missing gap data within your scope.')+'</div>');}
    var ov=document.createElement('div');ov.id='_gapApprovalsOv';ov.style.cssText='position:fixed;inset:0;z-index:2147483647;background:rgba(15,23,42,.45);backdrop-filter:blur(7px);display:flex;align-items:center;justify-content:center;padding:22px;direction:'+(A?'rtl':'ltr');
    ov.innerHTML='<div style="width:min(930px,96vw);max-height:86vh;overflow:auto;background:linear-gradient(135deg,rgba(255,255,255,.94),rgba(248,250,252,.86));border:1px solid rgba(255,255,255,.72);border-radius:22px;box-shadow:0 28px 80px rgba(15,23,42,.28);padding:22px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><div style="font-size:16px;font-weight:900;color:#0f172a">'+esc(title)+'</div><button onclick="document.getElementById(\'_gapApprovalsOv\').remove()" style="width:34px;height:34px;border:1px solid rgba(148,163,184,.25);background:rgba(255,255,255,.75);border-radius:10px;cursor:pointer;color:#334155;font-size:16px">×</button></div>'+html+'</div>';
    document.body.appendChild(ov);ov.onclick=function(e){if(e.target===ov)ov.remove();};if(focusId)setTimeout(function(){var el=$('gap_apr_'+focusId);if(el){el.scrollIntoView({block:'center'});el.style.outline='3px solid rgba(1,149,175,.35)';}},100);
  };
  function removeBanner(){var b=$('qumcApprovalStatusBanner');if(b)b.remove();}
  setInterval(removeBanner,800);setTimeout(removeBanner,80);setTimeout(removeBanner,600);setTimeout(removeBanner,1600);
  var oldRender=window.renderCurrent;if(typeof oldRender==='function'&&!oldRender.__qumcNoOwnerBanner){var wrap=function(){var r=oldRender.apply(this,arguments);setTimeout(removeBanner,50);setTimeout(removeBanner,250);return r;};wrap.__qumcNoOwnerBanner=true;window.renderCurrent=wrap;}
})();
