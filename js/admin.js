...
(function(){
  ...
  window.showKpoGapStatusPopup=function(){
    ...
    var html='<style>.gap-apr-btn{border:none;border-radius:10px;padding:8px 14px;font-size:11px;font-weight:900;cursor:pointer}.gap-apr-btn.ok{background:#047857;color:#fff}.gap-apr-btn.warn{background:#D97706;color:#fff}.gap-apr-btn.bad{background:#B91C1C;color:#fff}</style>';
    if(isSuper()||isManager()){
      var reqs=approvalPendingItems(); if(title){title.textContent=a?'حالة طلبات الاعتماد':'Approval Requests Status';title.setAttribute('data-en','Approval Requests Status');title.setAttribute('data-ar','حالة طلبات الاعتماد');}
    ...
    if(isOwner()){
      var miss=ownerMissingItems(); if(title){title.textContent=a?'حالة تحليل الفجوات الخاصة بك':'Your Gap Analysis Status';title.setAttribute('data-en','Your Gap Analysis Status');title.setAttribute('data-ar','حالة تحليل الفجوات الخاصة بك');}
      html+=statusBodyCard(miss.length,a?'حالة إدخال بيانات الفجوات':'Your gap analysis status',a?'جميع بيانات الفجوات مكتملة':'All gap analysis data is up to date');
...
(function(){
  ...
  function donutChart(title,items){items=(items||[]).filter(function(x){return Number(x.value||0)>0;});var total=items.reduce(function(a,x){return a+Number(x.value||0);},0),palette=['#0195AF','#16A34A','#DC2626','#D97706','#7C3AED','#2563EB'],cursor=0,stops=[];items.forEach(function(x,i){var pct=total?Number(x.value||0)*100/total:0,next=cursor+pct,color=x.color||palette[i%palette.length];stops.push(color+' '+cursor.toFixed(2)+'% '+next.toFixed(2)+'%');cursor=next;});var bg=total?'conic-gradient('+stops.join(',')+')':'#E7EEF2';return'<div class="perf-ac-insight perf-ac-donut-card"><h3>'+escPerf(title)+'</h3><div class="perf-ac-donut-layout"><div class="perf-ac-donut" style="background:'+bg+'"><div><strong>'+total+'</strong><small>Total</small></div></div><div class="perf-ac-donut-legend">'+(items.length?items.map(function(x,i){return'<div><i style="background:'+(x.color||palette[i%palette.length])+'"></i><span>'+escPerf(x.label)+'</span><b>'+Number(x.value||0)+'</b></div>';}).join(''):'<div class="perf-ac-empty compact">No data</div>')+'</div></div></div>';}
  ...
  window._perfAdminRenderRequestAnalytics=function(rows){
    var page=document.getElementById('ap-requestanalytics');if(!page)return;
    var total=rows.length,pending=0,approved=0,rejected=0,rated=0,notRated=0,sum=0;
    rows.forEach(function(r){var st=statusKey(r.status),n=Number(r.rating||0);if(st==='pending'||st==='open')pending++;if(st==='approved')approved++;if(st==='rejected')rejected++;if(n>0){rated++;sum+=n;}else if(st==='approved'||st==='rejected')notRated++;});
    var avg=rated?(sum/rated).toFixed(1):'0.0';
    var head='<div class="perf-ac-head"><div><h2>User Request Analytics</h2><p>Performance request outcomes, ratings and user feedback.</p></div><button class="perf-ac-refresh" onclick="window._perfAdminLoadRequestAnalytics()">Refresh</button></div>';
    var metrics='<div class="perf-ac-metrics">'+metric('Total Requests',total,'info','All submitted')+metric('Pending',pending,'warn','Awaiting decision')+metric('Approved',approved,'good','Completed approvals')+metric('Rejected',rejected,'bad','Final rejections')+'</div><div class="perf-ac-metrics perf-ac-metrics-secondary">'+metric('Rated',rated,'purple','User feedback')+metric('Not Rated',notRated,'warn','Completed without feedback')+metric('Average Rating',avg+' / 5','rating',rated+' rating(s)')+'</div>';
    var insights='<div class="perf-ac-insight-grid perf-ac-insight-grid-two">'+donutChart('Request Outcomes',[{label:'Pending',value:pending,color:'#D97706'},{label:'Approved',value:approved,color:'#16A34A'},{label:'Rejected',value:rejected,color:'#DC2626'}])+miniChart('Requests by Type',countBy(rows,function(r){return r.requestType||'Other';}))+'</div>';
    var table='<div class="perf-ac-table-wrap"><table class="perf-ac-table"><thead><tr><th>Date</th><th>User / Department</th><th>Request Type</th><th>Status</th><th>Admin Response</th><th>Rating & Comment</th></tr></thead><tbody>'+(rows.length?rows.map(function(r){var st=statusKey(r.status),response=r.adminComment||r.superAdminComment||'—';return'<tr><td style="white-space:nowrap">'+escPerf(fmtPerfTs(r.createdAt))+'</td><td><b>'+escPerf(r.userName||r.userEmail||'—')+'</b><small>'+escPerf(r.userEmail||'')+(r.department?' · '+escPerf(r.department):'')+'</small></td><td>'+escPerf(r.requestType||'—')+'</td><td>'+statusPill(st)+'</td><td class="perf-ac-message">'+escPerf(response)+'</td><td>'+stars(r.rating)+(r.rating?'<b class="perf-ac-rating-number">'+Number(r.rating)+'/5</b>':'')+(r.ratingComment?'<div class="perf-ac-rating-comment">'+escPerf(r.ratingComment)+'</div>':'')+'</td></tr>';}).join(''):'<tr><td colspan="6"><div class="perf-ac-empty">No requests yet.</div></td></tr>')+'</tbody></table></div>';
  ...
  window._perfAdminRenderReviewAnalytics=function(rows){
    var page=document.getElementById('ap-reviewanalytics');if(!page)return;
    var total=rows.length,newCount=0,editCount=0,open=0,progress=0,closed=0,rated=0,notRated=0,sum=0;
    rows.forEach(function(r){var t=String(r.requestType||'').toLowerCase(),s=statusKey(r.status),n=Number(r.rating||0),rateEligible=['closed','completed','approved','rejected','duplicate','out_of_scope','knowledge_guide'].indexOf(s)>=0;if(t==='new')newCount++;else editCount++;if(s==='open'||s==='pending')open++;else if(s==='in_progress'||s==='responded'||s==='under_review')progress++;else if(['closed','completed','approved','rejected','cancelled','duplicate','out_of_scope','knowledge_guide'].indexOf(s)>=0)closed++;if(n>0){rated++;sum+=n;}else if(rateEligible)notRated++;});
    var avg=rated?(sum/rated).toFixed(1):'0.0',rate=total?Math.round(closed*100/total):0;
    ...
    var metrics='<div class="perf-ac-metrics">'+metric('Total',total,'info','All review requests')+metric('New Item',newCount,'purple','New record requests')+metric('Edit / Review',editCount,'info','Existing item reviews')+metric('Open',open,'bad','Not started')+metric('In Progress',progress,'warn','Under review')+metric('Closed',closed,'good','Completed or closed')+'</div><div class="perf-ac-metrics perf-ac-metrics-secondary" style="grid-template-columns:repeat(4,minmax(0,1fr))!important">'+metric('Rated',rated,'purple','User feedback')+metric('Not Rated',notRated,'warn','Closed without feedback')+metric('Average Rating',avg+' / 5','rating',rated+' rating(s)')+metric('Closure Rate',rate+'%','good','Closed / total')+'</div>';
    var insights='<div class="perf-ac-insight-grid perf-ac-insight-grid-three">'+donutChart('Current Status Distribution',[{label:'Open',value:open,color:'#DC2626'},{label:'In Progress',value:progress,color:'#D97706'},{label:'Closed',value:closed,color:'#16A34A'}])+miniChart('Requests by Department',countBy(rows,function(r){return r.departmentKey||r.department||'Unassigned';}))+miniChart('Requests by Item Type',countBy(rows,function(r){return r.category||r.relatedType||'Other';}))+'</div>';
    var table='<div class="perf-ac-table-wrap"><table class="perf-ac-table"><thead><tr><th>Request</th><th>Requester / Department</th><th>Type / Item</th><th>Status</th><th>Rating & Comment</th><th>Last Update</th></tr></thead><tbody>'+(rows.length?rows.map(function(r){var st=statusKey(r.status),typ=String(r.requestType||'')==='new'?'New Item Request':'Existing Item Review & Update';return'<tr><td><b>'+escPerf(r.code||r.id||'—')+'</b><small>'+escPerf(r.title||'')+'</small></td><td>'+escPerf(r.userName||r.userEmail||'—')+'<small>'+escPerf(r.departmentKey||r.department||'—')+'</small></td><td>'+escPerf(typ)+'<small>'+escPerf(r.category||r.relatedType||'—')+'</small></td><td>'+statusPill(st)+'</td><td>'+stars(r.rating)+(r.rating?'<b class="perf-ac-rating-number">'+Number(r.rating)+'/5</b>':'')+(r.ratingComment?'<div class="perf-ac-rating-comment">'+escPerf(r.ratingComment)+'</div>':'')+'</td><td style="white-space:nowrap">'+escPerf(fmtPerfTs(r.updatedAt||r.createdAt))+'</td></tr>';}).join(''):'<tr><td colspan="6"><div class="perf-ac-empty">No Review & Development requests yet.</div></td></tr>')+'</tbody></table></div>';
    page.innerHTML=head+metrics+insights+table;
  };
