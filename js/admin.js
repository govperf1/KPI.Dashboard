...
(function(){
  ...
  window._perfAdminRenderRequestAnalytics=function(rows){
    ...
    var total=rows.length,pending=0,approved=0,rejected=0,rated=0,sum=0;
    rows.forEach(function(r){var st=statusKey(r.status);if(st==='pending'||st==='open')pending++;if(st==='approved')approved++;if(st==='rejected')rejected++;var n=Number(r.rating||0);if(n>0){rated++;sum+=n;}});
    ...
    var metrics='<div class="perf-ac-metrics">'+metric('Total Requests',total,'info','All submitted')+metric('Pending',pending,'warn','Awaiting decision')+metric('Approved',approved,'good','Completed approvals')+metric('Rejected',rejected,'bad','Final rejections')+metric('Rated',rated,'purple','User feedback')+metric('Average Rating',avg+' / 5','rating',rated+' rating(s)')+'</div>';
  ...
  window._perfAdminRenderReviewAnalytics=function(rows){
    ...
    var total=rows.length,newCount=0,editCount=0,open=0,progress=0,closed=0,rated=0,sum=0;
    rows.forEach(function(r){var t=String(r.requestType||'').toLowerCase(),s=statusKey(r.status);if(t==='new')newCount++;else editCount++;if(s==='open'||s==='pending')open++;else if(s==='in_progress'||s==='responded'||s==='under_review')progress++;else if(['closed','completed','approved','rejected','cancelled'].indexOf(s)>=0)closed++;var n=Number(r.rating||0);if(n>0){rated++;sum+=n;}});
