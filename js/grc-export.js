/* ======================================================================
   ...
   Build: 2026-07-28 v75 request delivery and executive summary report
   ====================================================================== */
(function(){
  'use strict';
  ...
  function sectionHead(number,title,description,selectedLabels){return'<div class="grc-exec-section-head"><span>'+number+'</span><div><small>EXECUTIVE SECTION '+String(number).padStart(2,'0')+'</small><h2>'+esc(title)+'</h2><p>'+esc(description)+'</p>'+(selectedLabels&&selectedLabels.length?'<div class="grc-exec-tags">'+selectedLabels.map(function(x){return'<b>'+esc(x)+'</b>';}).join('')+'</div>':'')+'</div></div>';}
  ...
  function complianceSummary(list){list=Array.isArray(list)?list:[];var applicable=list.filter(function(r){var s=String(r.complianceStatus||'').trim().toLowerCase();return s&&s!=='not applicable';}),met=applicable.filter(function(r){var s=String(r.complianceStatus||'');return /fully met|\bmet\b|compliant/i.test(s)&&!/not met|non.?compliant/i.test(s);}).length,partial=applicable.filter(function(r){return /partial/i.test(String(r.complianceStatus||''));}).length,notMet=Math.max(0,applicable.length-met-partial);return{total:list.length,applicable:applicable.length,met:met,partial:partial,notMet:notMet,rate:applicable.length?Math.round(met/applicable.length*100):0};}
  ...
  function governanceSection(data,number,labels){var all=data.policies.concat(data.plans,data.forms),positive=all.filter(function(r){return positiveStatus(reportStatus(r));}).length,attention=all.filter(function(r){return attentionStatus(reportStatus(r));}).length,types={Policies:data.policies.length,Plans:data.plans.length,Forms:data.forms.length},status=countBy(all);return'<section class="grc-exec-section">'+sectionHead(number,'Governance','Controlled policies, plans and forms that define how Facility Management & Safety work is governed and documented.',labels)+'<div class="grc-exec-metrics">'+metric('Governance Records',all.length,'Selected controlled records','navy')+metric('Active / Valid',positive,'Records in a positive current status','teal')+metric('Needs Attention',attention,'Expired, pending or invalid records','amber')+metric('Current Rate',all.length?Math.round(positive/all.length*100)+'%':'—','Positive records as a share of total','purple')+'</div><div class="grc-exec-chart-grid">'+barChart('Records by Type',types,'teal')+donutChart('Governance Status',status,all.length)+'</div></section>';}
  function riskSection(data,number,labels){var severity={Low:0,Medium:0,High:0,Critical:0};data.risks.forEach(function(r){var score=Number(r.riskScore)||Number(r.likelihood||0)*Number(r.impact||0),level=score>=15?'Critical':score>=8?'High':score>=4?'Medium':'Low';severity[level]++;});var openInc=data.incidents.filter(function(r){return !/closed|complete/i.test(reportStatus(r));}).length,critical=severity.Critical,high=severity.High;var typeCounts={'Risk Register':data.risks.length,'Incidents':data.incidents.length,'Emergency Codes':data.codes.length};return'<section class="grc-exec-section">'+sectionHead(number,'Risk Management','The current exposure profile, incident activity and emergency-code records used to support risk-based decisions.',labels)+'<div class="grc-exec-metrics">'+metric('Total Risks',data.risks.length,'Risks in the selected scope','navy')+metric('Critical Risks',critical,'Risk score 15–25','red')+metric('High Risks',high,'Risk score 8–14','amber')+metric('Open Incidents',openInc,'Incidents not closed or completed','purple')+'</div><div class="grc-exec-chart-grid grc-exec-chart-grid-wide">'+barChart('Risk Severity Distribution',severity,'risk')+reportHeatMap(data.risks)+'</div><div class="grc-exec-chart-grid">'+barChart('Risk & Event Records',typeCounts,'teal')+donutChart('Incident Status',countBy(data.incidents),data.incidents.length)+'</div></section>';}
  function complianceSection(data,number,labels){var c=complianceSummary(data.cbahi),j=complianceSummary(data.jci),app=c.applicable+j.applicable,met=c.met+j.met,overall=app?Math.round(met/app*100):0,authority={CBAHI:c.rate,JCI:j.rate},outcomes={Met:c.met+j.met,Partial:c.partial+j.partial,'Not Met':c.notMet+j.notMet};return'<section class="grc-exec-section">'+sectionHead(number,'Compliance','A consolidated view of CBAHI and JCI assessment results, excluding Not Applicable requirements from rate calculations.',labels)+'<div class="grc-exec-metrics">'+metric('Overall Compliance',overall+'%','Met requirements ÷ applicable requirements','teal')+metric('CBAHI Rate',c.rate+'%',c.applicable+' applicable requirements','navy')+metric('JCI Rate',j.rate+'%',j.applicable+' applicable requirements','purple')+metric('Open Compliance Gap',c.notMet+j.notMet+(c.partial+j.partial),'Not met and partially met requirements','amber')+'</div><div class="grc-exec-chart-grid">'+barChart('Compliance Rate by Authority',authority,'teal')+donutChart('Assessment Outcomes',outcomes,app)+'</div></section>';}
  function reportsSection(data,number,labels){var status=countBy(data.reports),families=countBy(data.reports,function(r){return r.family||r.type||'Other';}),available=data.reports.filter(function(r){return /available|complete|published/i.test(reportStatus(r));}).length,currentYear=new Date().getFullYear(),yearCount=data.reports.filter(function(r){return Number(r.year)===currentYear;}).length;return'<section class="grc-exec-section">'+sectionHead(number,'Reports','Availability and distribution of approved Facility Management & Safety reports in the platform.',labels)+'<div class="grc-exec-metrics">'+metric('Total Reports',data.reports.length,'Reports in the selected scope','navy')+metric('Available',available,'Reports available for viewing','teal')+metric('Current Year',yearCount,'Reports registered for '+currentYear,'purple')+metric('Availability Rate',data.reports.length?Math.round(available/data.reports.length*100)+'%':'—','Available reports ÷ total reports','amber')+'</div><div class="grc-exec-chart-grid">'+barChart('Reports by Family',families,'teal')+donutChart('Report Availability',status,data.reports.length)+'</div></section>';}
  function manualsSection(data,number,labels){var status=countBy(data.manuals),languages=countBy(data.manuals,function(r){return r.language||'Not Specified';}),available=data.manuals.filter(function(r){return /available|active|valid|published/i.test(reportStatus(r));}).length;return'<section class="grc-exec-section">'+sectionHead(number,'Manuals & Guidelines','Controlled manuals and guidance documents available to support consistent operational practice.',labels)+'<div class="grc-exec-metrics">'+metric('Total Documents',data.manuals.length,'Manuals and guidelines','navy')+metric('Available',available,'Documents currently available','teal')+metric('Arabic',Number(languages.Arabic||languages.AR||0),'Arabic-language documents','purple')+metric('English',Number(languages.English||languages.EN||0),'English-language documents','amber')+'</div><div class="grc-exec-chart-grid">'+barChart('Documents by Language',languages,'teal')+donutChart('Document Status',status,data.manuals.length)+'</div></section>';}
  function initiativesSection(data,number,labels){var status=countBy(data.initiatives),departments=countBy(data.initiatives,initiativeDepartment),completed=data.initiatives.filter(function(r){return /complete|completed|closed/i.test(reportStatus(r));}).length,active=data.initiatives.filter(function(r){return /active|progress|ongoing|open/i.test(reportStatus(r));}).length,avg=data.initiatives.length?Math.round(data.initiatives.reduce(function(n,r){return n+Number(r.progress||0);},0)/data.initiatives.length):0;return'<section class="grc-exec-section">'+sectionHead(number,'Initiatives','Improvement initiatives, their implementation status and distribution across Facility Management & Safety departments.',labels)+'<div class="grc-exec-metrics">'+metric('Total Initiatives',data.initiatives.length,'Registered initiatives','navy')+metric('Completed',completed,'Completed or closed initiatives','teal')+metric('In Progress',active,'Active or ongoing initiatives','amber')+metric('Average Progress',avg+'%','Average recorded implementation progress','purple')+'</div><div class="grc-exec-chart-grid">'+barChart('Initiatives by Department',departments,'teal')+donutChart('Initiative Status',status,data.initiatives.length)+'</div></section>';}
  function buildExecutiveReport(items,depts){var data=reportData(items,depts),date=new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'long',year:'numeric'}),doc=document.createElement('div'),sections=[],n=1;
    REPORT_GROUPS.forEach(function(group){var labels=selectedLabels(group,items);if(!labels.length)return;if(group.id==='governance')sections.push(governanceSection(data,n++,labels));else if(group.id==='risk')sections.push(riskSection(data,n++,labels));else if(group.id==='compliance')sections.push(complianceSection(data,n++,labels));else if(group.id==='reports')sections.push(reportsSection(data,n++,labels));else if(group.id==='manuals')sections.push(manualsSection(data,n++,labels));else if(group.id==='initiatives')sections.push(initiativesSection(data,n++,labels));});
    ...
    doc.id='rptDocument';doc.className='grc-executive-summary-report';doc.innerHTML='<div class="grc-exec-report">'+reportHeader('GRC Executive Summary','Facility Management & Safety Division',date,departmentText(depts))+'<main class="grc-exec-report-body"><section class="grc-exec-overview"><div><small>EXECUTIVE OVERVIEW</small><h2>Governance, Risk &amp; Compliance at a Glance</h2><p>This report consolidates the selected platform indicators into one executive summary. All results are calculated from the current registers and assessment data for the selected department scope.</p></div><div class="grc-exec-overview-grid">'+metric('Governance Records',totalGovernance,'Policies, plans and forms','navy')+metric('Registered Risks',totalRisk,'Current risk register','amber')+metric('Compliance Rate',comp.rate+'%','Combined applicable CBAHI and JCI items','teal')+metric('Supporting Outputs',totalOperational,'Reports, manuals and initiatives','purple')+'</div></section>'+sections.join('')+'</main>'+reportFooter(date)+'</div>';
    var style=document.createElement('style');style.textContent=`
@media print{@page{size:A4 landscape;margin:8mm}.grc-exec-section{break-before:page}.grc-exec-section:first-of-type{break-before:auto}.grc-exec-chart,.grc-exec-metric,.grc-exec-section-head{break-inside:avoid}.grc-exec-report-header,.grc-exec-report-footer{break-inside:avoid}}
#rptDocument{font-family:Arial,"Segoe UI",sans-serif;color:#152538;background:#fff}.grc-exec-report{background:#fff}.grc-exec-report-header{display:flex;align-items:center;gap:18px;padding:16px 24px 13px;background:#152538;color:#fff;border-radius:10px 10px 0 0}.grc-exec-logo img{height:54px;max-width:150px;object-fit:contain}.grc-exec-heading{flex:1;border-left:2px solid rgba(1,181,210,.6);padding-left:18px}.grc-exec-heading>span{display:block;font-size:7px;font-weight:900;letter-spacing:.17em;color:#01c0dc}.grc-exec-heading h1{margin:3px 0;font-size:20px;color:#fff}.grc-exec-heading p{margin:0 0 7px;font-size:9px;color:rgba(255,255,255,.78)}.grc-exec-heading>div{display:flex;gap:8px;font-size:7px;color:#01c0dc}.grc-exec-header-line{height:4px;background:linear-gradient(90deg,#0195af,#01c5e8 45%,#152538)}.grc-exec-report-body{padding:14px 16px 16px}.grc-exec-overview{padding:14px;border:1px solid #d8e6ea;border-radius:12px;background:linear-gradient(135deg,#f4fafb,#fff);margin-bottom:14px}.grc-exec-overview small,.grc-exec-section-head small{font-size:6.5px;font-weight:900;letter-spacing:.14em;color:#0195af}.grc-exec-overview h2{margin:4px 0 5px;font-size:17px}.grc-exec-overview p,.grc-exec-section-head p{margin:0;color:#647a87;font-size:8px;line-height:1.55}.grc-exec-overview-grid,.grc-exec-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px;margin-top:12px}.grc-exec-section{padding:10px 0 2px;min-height:164mm}.grc-exec-section-head{display:flex;gap:11px;align-items:flex-start;padding:10px 12px;border:1px solid #d9e6ea;border-left:5px solid #0195af;border-radius:10px;background:#f8fbfc}.grc-exec-section-head>span{display:grid;place-items:center;flex:0 0 38px;height:38px;border-radius:9px;background:#152538;color:#fff;font-size:16px;font-weight:900}.grc-exec-section-head h2{margin:2px 0 4px;font-size:15px}.grc-exec-tags{display:flex;gap:5px;flex-wrap:wrap;margin-top:7px}.grc-exec-tags b{padding:3px 7px;border-radius:999px;background:#e9f5f7;color:#087a90;font-size:6px}.grc-exec-metric{border:1px solid #dbe7eb;border-top:4px solid #153f55;border-radius:10px;padding:10px 11px;background:#fff;min-height:58px}.grc-exec-metric.teal{border-top-color:#0195af}.grc-exec-metric.amber{border-top-color:#d89a2b}.grc-exec-metric.purple{border-top-color:#7a5aa6}.grc-exec-metric.red{border-top-color:#cf5f69}.grc-exec-metric span{display:block;font-size:7px;font-weight:800;color:#617987}.grc-exec-metric strong{display:block;margin:3px 0;font-size:19px;color:#153f55}.grc-exec-metric small{font-size:6px;color:#82939d}.grc-exec-chart-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px;margin-top:10px}.grc-exec-chart-grid-wide{grid-template-columns:minmax(0,.8fr) minmax(0,1.2fr)}.grc-exec-chart{border:1px solid #dbe7eb;border-radius:11px;padding:10px 11px;background:#fbfdfe;min-height:105px}.grc-exec-chart-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:9px}.grc-exec-chart-head h3{margin:0;font-size:9px;color:#153f55}.grc-exec-chart-head span{font-size:6px;color:#7c909b}.grc-exec-bars{display:grid;gap:6px}.grc-exec-bars>div{display:grid;grid-template-columns:105px 1fr 26px;gap:7px;align-items:center}.grc-exec-bars label{font-size:6.5px;color:#4c6573;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.grc-exec-bars i{height:8px;background:#e7eff2;border-radius:999px;overflow:hidden}.grc-exec-bars i b{display:block;height:100%;background:linear-gradient(90deg,#0195af,#00bfd5);border-radius:999px}.grc-exec-bars strong{font-size:7px;text-align:right}.grc-exec-donut-wrap{display:grid;grid-template-columns:105px 1fr;gap:14px;align-items:center}.grc-exec-donut{width:98px;height:98px;border-radius:50%;display:grid;place-items:center}.grc-exec-donut>div{width:61px;height:61px;border-radius:50%;background:#fff;display:grid;place-items:center;align-content:center;box-shadow:inset 0 0 0 1px #dce8eb}.grc-exec-donut strong{font-size:17px}.grc-exec-donut span{font-size:6px;color:#718691}.grc-exec-legend{display:grid;gap:5px}.grc-exec-legend>div{display:grid;grid-template-columns:8px 1fr 24px;gap:6px;align-items:center;font-size:6.5px}.grc-exec-legend i{width:8px;height:8px;border-radius:3px}.grc-exec-legend strong{text-align:right}.grc-exec-heat-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:4px}.grc-exec-heat-grid>div{min-height:31px;border-radius:6px;padding:4px;display:grid;grid-template-columns:1fr auto;align-items:center}.grc-exec-heat-grid .low{background:#a8d7c4}.grc-exec-heat-grid .medium{background:#f1dc85}.grc-exec-heat-grid .high{background:#efaa73}.grc-exec-heat-grid .critical{background:#d96d73;color:#fff}.grc-exec-heat-grid small,.grc-exec-heat-grid span{font-size:5px}.grc-exec-heat-grid strong{font-size:9px}.grc-exec-heat-legend{display:flex;gap:6px;margin-top:7px}.grc-exec-heat-legend span{padding:3px 7px;border-radius:999px;font-size:5.5px}.grc-exec-heat-legend .low{background:#a8d7c4}.grc-exec-heat-legend .medium{background:#f1dc85}.grc-exec-heat-legend .high{background:#efaa73}.grc-exec-heat-legend .critical{background:#d96d73;color:#fff}.grc-exec-empty{font-size:7px;color:#7a8e99}.grc-exec-report-footer{display:flex;justify-content:space-between;align-items:center;padding:10px 18px;background:#152538;color:#fff;border-radius:0 0 10px 10px}.grc-exec-report-footer div{display:flex;flex-direction:column;gap:2px}.grc-exec-report-footer strong,.grc-exec-report-footer b{font-size:8px}.grc-exec-report-footer span,.grc-exec-report-footer small{font-size:6px;color:rgba(255,255,255,.7)}
`;doc.appendChild(style);return doc;}
  window._grcGenerateReport=async function(){var items=checked('grcReportItem'),depts=REPORT_FIXED_DEPTS.length?REPORT_FIXED_DEPTS:checked('grcReportDept');if(!items.length){alert('Select at least one report content option.');return;}if(!depts.length){alert('Select at least one department.');return;}var doc=buildExecutiveReport(items,depts);closeOverlay();var period=(depts.length===DEPTS.length?'All Departments':depts.map(departmentTitle).join(', '))+' · '+new Date().toLocaleDateString('en-GB'),ok=typeof window._qumcPrintReportDocument==='function'&&window._qumcPrintReportDocument(doc,{period:'GRC Executive Summary · '+period,orientation:'landscape'});if(!ok){alert('The Performance report print engine is not ready. Reload the page and try again.');return;}if(typeof window.addAudit==='function')window.addAudit('GRC_REPORT_PDF','Generated GRC executive summary: '+items.join(', '));};
